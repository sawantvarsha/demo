...
export class RmwComponent implements OnInit, OnDestroy {
  @Input() control: any;
  @Input() data: any;
  saveFilter:any = [];
  FormData = [];
  ...
  ngOnInit(): void {
    ...
    // this.rateFilterString =  "and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation') and RIGHT(NM.Note_Product_Rating, 1) between '1' and '" + (this.homeapi.RiskRating?.split('')[1] || '5') + "'"; //Added by Alolika | 25-02-2022
    this.rateFilterString = "and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation')"; //Added by Alolika | 25-02-2022
    ...
    this.afs.getCommonDataFXOrder('CSPRMWProducts').subscribe((res) => {
      if (res.length !== 0) {
        ...
        if (this.stateFlag) {
          ...
          this.pageNo = 1;
          this.pageSize = 5;
          this.changeProduct('EquityAutocallableNote', this.tabs.length);
        }
      }
    });
    ...
    this.workflowApi.RMWSortingDataObserver.subscribe((res: any) => {
      if (res.length !== 0) {
        ...
        if (this.sort === '') {
          ...
          this.selectedTarget = this.sortingData[0].RSM_Target_Field;
        }
      }
    });
  ...
  changeProduct(choice, index) {
    ...
    } else if (this.choice === 'EquityAutocallableNote') {
      this.templateName = 'SP';
    }
  ...
  getSavedFilters(choice) {
    switch (choice) {
      ...
      case 'SP':
        ...
        this.filterString = this.rateFilterString + this.filterString; //Added by Alolika | 04-08-2022
      ...
      case 'ALL':
        ...
        this.whereclause =
          "Where Template_Code in ( 'Bonds','Fund_Setup','Product_Maintenance','NiftyParticipation', 'EquityAutocallableNote')",
      ...
      default:
        this.saveFilter = [];
        // this.filterString = "and RIGHT(NM.Note_Product_Rating, 1) < " + this.homeapi.RiskRating.split('')[1]; //Added by Alolika | 17-02-2022;
        this.filterString = "and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation')"; //Added by Alolika | 17-02-2022
        // this.filterString = "and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation') and RIGHT(NM.Note_Product_Rating, 1) between '1' and '" + (this.homeapi.RiskRating?.split('')[1] || '5') + "'"; //Added by Alolika | 17-02-2022
        this.ccy = '';
        this.foldersString = '';
        this.ListType = '';
        this.searchBarString = '';
  ...
  searchPopup() {
    ...
    if (this.searchFlag === true) {
      ...
      if (this.selectedProduct === 'ALL') {
        ...
        if (this.selectedProduct === 'EquityAutocallableNote') {
        ...
        this.workflowApi.getRMWFilter(this.spId);
      } else if (this.selectedProduct === 'EquityAutocallableNote') {
        this.workflowApi.getRMWFilter(this.spId);
      }
    } else {
      // this.filterString = '';
  ...
  addFilter() {
    ...
    //     // this.saveFilter.push(['Maturity Date', this.fromDate + '-' + this.toDate, 'NM.Maturity_Date', 'slider']);
  ...
  saveRmwFilters() {
    ...
    if (this.showbonds) {
      ...
      this.folderName = '';
      this.liststring = '';
      this.stringBonds = this.filterString.replace("and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation')", ""); //Added by Alolika | 25-02-2022
      ...
      this.liststring1 = '';
      this.stringFunds = '';
      this.stringFunds = this.filterString.replace("and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation')", "");//Added by Alolika | 25-02-2022
      ...
      this.folderName2 = '';
      this.liststring2 = '';
      this.stringInsurance = this.filterString.replace("and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation') and RIGHT(NM.Note_Product_Rating, 1) between '1' and '" + (this.homeapi.RiskRating?.split('')[1] || '5') + "'", "");//Added by Alolika | 25-02-2022
      ...
      this.folderName3 = '';
      this.liststring3 = '';
      this.stringAll = this.filterString.replace("and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation') and RIGHT(NM.Note_Product_Rating, 1) between '1' and '" + (this.homeapi.RiskRating?.split('')[1] || '5') + "'", "");//Added by Alolika | 25-02-2022
      ...
      this.folderName4 = '';
      this.liststring4 = '';
      this.stringNifty = this.filterString.replace("and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation')", "");//Added by Alolika | 25-02-2022
      ...
      this.folderName5 = '';
      this.liststring5 = '';
      this.stringSP = this.filterString.replace("and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation')", "");//Added by Alolika | 25-02-2022
  ...
  selectDateEvent(item, min, max, header) {
    let flag = false;
    if (this.saveFilter.length > 0) {
      this.saveFilter.forEach((element) => {
        if (element[2] === item) {
          ...
          if (!element[1].includes(header)) {
            this.saveFilter.push([header, min + ' - ' + max, item, 'date']);
          }
          return;
        }
      });
    ...
    if (!this.filterString.includes(item)) {
      ...
      // this.filterString = this.filterString.replace(
      //   this.filterString.split(item)[1].split('and ')[1],
      //   " and '" + max + "'"
      // );
    }
  }
  ...
  getRMWSearch(searchBarString) {
    if(this.filterString.includes(this.rateFilterString)) {
      this.filterString = this.filterString.replace(this.rateFilterString, "");
    }
  ...
  selectDate(date) {
    ...
    // if(dir === 'from') {
    //   from = date;
    //   this.saveFilter.push([header, this.datepipe.transform(from, 'dd-MMM-yyyy') + '-' + this.datepipe.transform(from, 'dd-MMM-yyyy'), item, 'slider']);
    // } else if(dir === 'to') {
    //   to = date;
    //   this.saveFilter.push([header, this.datepipe.transform(from, 'dd-MMM-yyyy') + '-' + this.datepipe.transform(to, 'dd-MMM-yyyy'), item, 'slider']);
    // }
  ...
  removeFilter(item, filterArr: any[]) {
    ...
    if (this.filterString.includes(item[2])) {
      ...
      } else if (item[this.counter] === 'date') {
        splitarray.forEach((element, i) => {
          if (element.includes(item[2])) {
            this.filterString = this.filterString.replace('and' + element, '');
            this.filterString = this.filterString.replace('and' + splitarray[i + 1], '');
          }
        });
      } else {
        this.filterString = this.filterString.replace(
  ...
  refreshPage() {
    ...
    this.pageNo = 1;
    this.pageSize = 5;
    this.filterString = "and RFM.RFMD_Layout_Code in ('Angular','Default','Lifecycle','Valuation')"; //Added by Alolika | 19-02-2022
  ...
  disableEvent(event: KeyboardEvent) {
