...
@Component({
  selector: 'app-ec-vertical-submultirequest',
  templateUrl: './ec-vertical-submultirequest.component.html',
  styleUrls: ['./ec-vertical-submultirequest.component.scss']
})
export class EcVerticalSubmultirequestComponent implements OnInit, OnDestroy  {
  ...
  constructor(public elem: ElementRef, public commonfunctions: EcCommonService,
    public apifunctions: EcHomeService, public datepipe: DatePipe) {
    try {
      this.flag = false;
      // this.shares = [];
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (this.templateMappingArr && this.templateMappingArr.length > 0) {
          ...
          // }
          this.defaultvaluesArr = this.apifunctions.GetEntityDefaultValues("", (this.commonfunctions.getLoggedInUserName())[0].UserId, this.templateMappingArr[0].template);
  ...
  ChangeIndex(_e) {
    try {
      ...
      console.log(this.shares, this.flag, this.showSuggestions)
    } catch (error) {
      //console.log('Error:', error);
    }
  }
        ...
        if (this.ShareBasket.length < 5) {
          if (this.ShareBasket.find(i => i.BloombergCode === item.BloombergCode) === undefined) {
            this.ShareBasket.push({
              BloombergCode: item.BloombergCode, Ccy: item.Ccy, LongName: item.LongName, RICCode: item.Code, Code: item.Code,
  ...
  PHXPrice() {
    try {
      ...
      const parameters = {
        ...
        LP: this.apifunctions.priceProviders.join(','),//'BBVA',
        xmlstr,
  ...
  txtTenorChange(e, type: any) {
    try {
      ...
      if (type === 'Strike') {
        ...
        // }
        // else {
        this.Dates = this.apifunctions.BBVAGetDates(this.Exchange(), str.toUpperCase(), '');
      ...
      if (type === 'Payment') {
        ...
        // }
        // else {
        this.Dates = this.apifunctions.BBVAGetDates(this.ddlNoteCcy, str.toUpperCase(), this.stkdate);
        strDate = this.commonfunctions.formatDate(this.Dates.MaturityDate);
        // this.datepipe.transform(this.Dates.MaturityDate, 'yyyy-MM-dd');
        // }
      ...
      if (type === 'Expiry') {
        ...
        if (str.toUpperCase() === '0B') {
          ...
          this.Dates = this.apifunctions.BBVAGetDates(this.ddlNoteCcy, str.toUpperCase(), this.settdate);
          strDate = this.commonfunctions.formatDate(this.Dates.MaturityDate);
          // this.datepipe.transform(this.Dates.MaturityDate, 'yyyy-MM-dd');
        }
  ...
  validationOnButton() {
    try {
      ...
      if (this.SolveForvalue !== 'IBPrice' && this.format !== 'Swap' && this.format !== 'Option') {
        const resMsg : any = this.apifunctions.reofferValidation(this.issuePrice, this.IBPrice, 'EQ', this.format);
        if (resMsg !== '') {
      ...
      if (this.autoNonCall !== '' && this.ProductName === 'AutocallablePhoenix') {
        const resMsg :any = this.apifunctions.noncallValidation(this.autoNonCall, this.expshift,
  ...
  PTCValidation() {
    try {
      ...
      this.checkNotionalRes = this.apifunctions.BBVACheckNotional('Participation', this.ddlNoteCcy);
  ...
  generatePTCXML(_userGroupID: any) {
    try {
      if (this.templateMappingArr && this.templateMappingArr.length > 0) {
        let xmlstr = '<QuoteRequest>';
        this.templateName = this.templateMappingArr[0].template;
  ...
  RequestTermsheet() {
    try {
      ...
      const errorMsg : any = this.apifunctions.termsheetSender(this.orderID, '', 'AutocallablePhoenix', 'AutocallablePhoenix', 'TS');
      if (errorMsg === '') {
        ...
        this.TSInterval = setInterval(() => {
          if (TSTimeLeft > 0) {
            const res : any = this.apifunctions.ViewTermsheet(that.sortedAllPrices[0].rfq, 'IndicativeTermsheet');
  ...
  GetTriggerValue(type: any) {
    ...
    if (type === 'autoTrigger') {
      this.TriggerValueArr = this.apifunctions.GetTriggerValues(this.expshift, this.autoFreq,
    ...
    if (type === 'fundRate') {
      if (this.ProductName === 'CreditTranche') {
        this.TriggerValueArr = this.apifunctions.GetTriggerValues(this.creditTenor, this.fundFreq,
          this.fundRate, '', '');
      } else {
  ...
  scheduleSend() {
    try {
      ...
      if (this.ErrorMsg === '') {
        ...
        const res = this.apifunctions.SchedulePrice(this.ProductName, xmlstr,
  ...
  RequestKID() {
    try {
      ...
      const errorMsg : any = this.apifunctions.termsheetSender(this.orderID, '', 'AutocallablePhoenix', 'AutocallablePhoenix', 'KID');
      if (errorMsg === '') {
        ...
        this.KIDInterval = setInterval(() => {
          if (TSTimeLeft > 0) {
            const res : any = this.apifunctions.ViewTermsheet(that.sortedAllPrices[0].rfq, 'KID Termsheet');
            if (res !== null && res !== undefined) {
              if (res.Status.toString().toUpperCase() === 'SUCCESS') {
                that.KIDFlag = true;
                TSTimeLeft = 0;
  ...
  ViewKID() {
    try {
      this.ErrorMsg = '';
      const res : any= this.apifunctions.ViewTermsheet(this.sortedAllPrices[0].rfq, 'KID Termsheet');
      if (res !== null && res !== undefined) {
        if (res.Status.toString().toUpperCase() === 'SUCCESS') {
  ...
  GenerateDocument() {
    try {
      ...
      const res: any = this.apifunctions.GenerateDocument(this.noteMasterID + '~' + this.rfqID);
      if (res !== null && res !== undefined) {
