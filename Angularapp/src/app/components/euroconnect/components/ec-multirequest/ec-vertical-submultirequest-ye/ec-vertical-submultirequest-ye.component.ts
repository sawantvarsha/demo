...
@Component({
  selector: 'app-ec-vertical-submultirequest-ye',
  templateUrl: './ec-vertical-submultirequest-ye.component.html',
  styleUrls: ['./ec-vertical-submultirequest-ye.component.scss']
})
export class EcVerticalSubmultirequestYeComponent implements OnInit {
  ...
  issuePrice: any = '';
  tempXML = '';
  tempXLJson = {}; // Bulk Pricer (YE) - Export to excel
  indexTranche = '';
  Detach: any = '';
  Attach: any = '';
  AttachVal: any = '';
  ...
  // FIN1EURINT-570 : Hamburger icon missing in vertical mode (Tooltip to display all fields) | 10-Aug-2023
  ...
  excelFlagSubscription: Subscription; // Bulk Pricer (YE) - Export to excel
  ...
  // { 'Key': 'Italy', 'Value': 'Italy' },//Commented by SandipA @18-Oct-23 for FIN1EURINT-657
  ...
  ngOnDestroy(): void {
    ...
    if (this.excelFlagSubscription){
      this.excelFlagSubscription?.unsubscribe();
    }
    ...
    try {
      ...
      $(document).on('click', function (event) {
        if (!$(event.target).closest('#menucontainer').length) {
          //that.closePopups() // commented by suvarna P || 09Apr2022 || Lp and price Popup issue
        }
      });
    } catch (error) {
  ...
  ngOnInit() {
    ...
    try {
      ...
      // FIN1EURINT-520 | Request for TS and KID links disable for LPs not supporting documentation
        ...
        if (this.templateMappingArr && this.templateMappingArr.length > 0) {
          ...
          this.defaultvaluesArr = await this.apifunctions.GetEntityDefaultValues("", (this.commonfunctions.getLoggedInUserName())[0].UserId, 'YieldEnhancement');
        ...
        this.phxSaveFlagVerticalSubscription = this.apifunctions.saveFlagCheck.subscribe(async flag => {
          if (flag) {
        ...
        this.phxPriceFlagVerticalSubscription = this.apifunctions.priceFlagCheck.subscribe(async flag => {
          if (flag) {
            this.togglesortedAllPrices = [];
	...
	if (this.showDefault) {
	  ...
	  // var idx = this.shares.findIndex(item => item.BloombergCode === this.data.ShareBBGRIC1);
          ...
          this.paymentshift = this.data.ComputedSettlementPeriodSoftTenor === '5B' ? 'T + 5' : this.data.ComputedSettlementPeriodSoftTenor === '10B' ? 'T + 10' : this.data.ComputedSettlementPeriodSoftTenor === '' ? 'Custom' : this.data.ComputedSettlementPeriodSoftTenor;
        ...
        this.Dates = await this.apifunctions.BBVAGetDates(this.ddlNoteCcy, (this.paymentshift === 'Custom' ? '' : (this.paymentshift === 'T + 10' ? '10B' : '5B')), this.stkdate);
          ...
          */
	      ...
	      // HSBCECCLI-10 : Code changes to filter shares for optimization and better performance | 10-Aug-2023
              //const shareCcy = this.shares[this.shares.findIndex(res => res.BloombergCode = this.SelectedUnderlyingBasketArray[0].BloombergCode)].Ccy;
	  ...
	  // FIN1EURINT-528 : Bulk Pricer - Issue Date custom input functionality like quick pricer
          ...
          // FIN1EURINT-206 : Yield Enhancement : Red Circle appears on the screen when Tenor is selected as Custom
  ...
  // renamed function to keep backup of old mappings || PriyaL || 07Apr2022 || Assigned by PranavD
        ...
        for (const i in this.templateMappingArr) {
          switch (this.templateMappingArr[i].email_Header) {
            case 'Product': xmlstr += '<' + this.templateMappingArr[i].template_Field_Name + '>'
              // + "Autocallable" + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              + "Yield Enhancement" + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              break;
    ...
    try {
     ...
     // Removed onbehalf dependency || PriyaL || 25Apr2022 || Assigned by Pranav D
            ...
            case 'loginUser': xmlstr += '<' + this.templateMappingArr[i].template_Field_Name + '>'
              ...
              + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              // FIN1EURINT-283 : Single pricer vs Multi price difference in Quote Request values | 04-Oct-2023
              break;
  ...
  // HSBCECCLI-21 : Add validation for Coupon p.a. - Zero coupons should not be allowed | 09-Aug-2023
  cpnValidation(e: any) {
    try {
      const target = this.commonfunctions.GetEventTarget(e);
      this.ErrorMsg = '';
  ...
  generateExcelJson(){
    ...
    this.tempXLJson = {
      ...
      'Put Gearing (%)' : this.putSpreadGearing,
      'Coupon Type' : this.cpnType,
      'Coupon Frequency' : this.cpnFreq,
      'Coupon p.a. (%)' : this.SolveForvalue === 'Coupon' || (this.ERCpnType !== 'None' && this.periodicCouponFlag === 'No') || (this.cpnType === '' || this.cpnType === 'Floating Unconditional' || this.cpnType === 'Floating without Memory') ? '' : this.cpnCoupon,
      ...
      switch (this.commonData[i].Field_Name) {
        ...
        case "IssueDateOffsetRef"://Sudarshan | Asked by vipul | 31-Jan-2024
          ...
          if (!(this.IssueDateOffset?.length > 0)){
            this.IssueDateOffset = this.parseCommonDatatoJSONArr('IssueDateOffsetRef');
          }
          break;
        case "KIDs":
 ...
 // function modified by AdilP || FIN1EURINT-502 || @28-06-2023
  autocallCouponTypeChange() {
    if (this.autocallCouponType === 'Constant Barrier' || this.autocallCouponType === 'Variable Barrier') {
      this.autoFreq = this.autoFreq || 'Monthly';
      this.callableFreq = '';
      this.putableFreq = '';
      ...
      this.autoFreq = '';
      this.callableFreq = this.callableFreq || 'Monthly';
      this.putableFreq = '';

    }
    if (this.autocallCouponType === 'Variable Barrier') {
      this.autoStepDown = this.autoStepDown || '0.00';
      this.autoTrigger = this.autoTrigger || '100.00';
    } else {
      this.autoStepDown = '';
      this.autoTrigger = '';
    }
    ...
    if (this.autocallCouponType === 'Constant Barrier') {
      this.autoTrigger = this.autoTrigger || '102.00';
    }
    this.autocallfreqbasedonTenor();
    this.autocallfrombasedonFreqnTenor();
  }
  ...
  autocallfrombasedonFreqnTenor() {
    ...
    if (this.expshift !== 'Custom') {
      ...
      if (this.autoFreq === 'Yearly' || this.autoFreq === 'Daily') {
        fromchar = 'Y'
  ...
  MinMaxValidation(e, Pricestr: string) {
    try {
      this.ERReset();
      this.enableToggle();
      const target = this.commonfunctions.GetEventTarget(e);
...
// Added by AdilP @16-06-2023 || FIN1EURINT-480 
  ...
  getSolveFor(str){
    ...
    else if(str === 'CouponBarrier'){
      return 'Coupon Barrier Level (%)';
    }
    else 
    {
      return '';
