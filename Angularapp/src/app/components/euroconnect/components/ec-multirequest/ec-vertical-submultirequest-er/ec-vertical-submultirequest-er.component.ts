...
@Component({
  selector: 'app-ec-vertical-submultirequest-er',
  templateUrl: './ec-vertical-submultirequest-er.component.html',
  styleUrls: ['./ec-vertical-submultirequest-er.component.scss']
})
export class EcVerticalSubmultirequestErComponent implements OnInit {
  ...
  defaultValueFlag = false; // declare new variable || PriyaL || 09Apr2022 || Assigned by Pranav D.
    ...
    try {
      ...
      // FIN1EURINT-520 | Request for TS and KID links disable for LPs not supporting documentation
        ...
        if (this.templateMappingArr && this.templateMappingArr.length > 0) {
          ...
          // this.defaultvaluesArr = this.apifunctions.GetEntityDefaultValues("", (this.commonfunctions.getLoggedInUserName())[0].UserId, 'EQC_Europe');
          this.defaultvaluesArr = await this.apifunctions.GetEntityDefaultValues("", AppConfig.settings.oRes.userID, 'EQC_Europe');       // Vaibhav B | Added UserID from session | 21-Mar-23


          if (this.defaultvaluesArr && this.defaultvaluesArr.length > 0) {
            this.setdefaultvalues();
            this.defaultValueFlag = false; // set default flag || PriyaL || 09Apr2022 || Assigned by Pranav D.
          }
        }
        ...
        this.priceProvidersArr = await this.apifunctions.GetPriceProviderDetails('EQC_Europe');
          ...
          */
          ...
          if (this.ProductName === 'AutocallablePhoenixER' || this.ProductName === 'YieldEnhancement' || this.ProductName === 'DiscountCertificate') {
            ...
            if (this.defaultvaluesArr && this.defaultvaluesArr.length > 0) {
              ...
              this.SelectedUnderlyingBasketArray = [];
              this.setdefaultvalues();
              //call function after setting default values || PriyaL || 09Apr2022 || Assigned by Pranav D.
  ...
  async showUnderlying(_event, item) {
    try {
      // default flag check || PriyaL || 09Apr2022 || Assigned by Pranav D.
      ...
      if (this.templateMappingArr && this.templateMappingArr.length > 0) {
	    ...
	    // HSBCECCLI-56 : Custom Autocall Changes in Bulk Pricer || 31-Aug-2023
            ...
            case 'LN5': xmlstr += '<' + this.templateMappingArr[i].template_Field_Name + '>'
              + (this.ShareBasket[4] === undefined || this.ShareBasket[4].LongName === undefined ? '' : this.commonfunctions.formatXMLString(this.ShareBasket[4].LongName))
              + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              break;
  ...
  generateExcelJson(){
    ...
    this.tempXLJson = {
      ...
      'Underlying1' : this.ShareBasket[0] ? this.ShareBasket[0].BloombergCode : '',
      'Underlying2' : this.ShareBasket[1] ? this.ShareBasket[1].BloombergCode : '',
      'Underlying3' : this.ShareBasket[2] ? this.ShareBasket[2].BloombergCode : '',
      'Underlying4' : this.ShareBasket[3] ? this.ShareBasket[3].BloombergCode : '',
      'Underlying5' : this.ShareBasket[4] ? this.ShareBasket[4].BloombergCode : '',
 ...
 // function modified by AdilP || FIN1EURINT-502 || @28-06-2023
  ...
  setdefaultvalues() {
    this.defaultValueFlag = true; // set default flag to handle reset functionality || PriyaL || 09Apr2022 || Assigned by Pranav D.
