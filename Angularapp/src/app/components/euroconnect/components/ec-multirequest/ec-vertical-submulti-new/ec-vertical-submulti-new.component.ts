...
@Component({
  selector: 'app-ec-vertical-submulti-new',
  templateUrl: './ec-vertical-submulti-new.component.html',
  styleUrls: ['./ec-vertical-submulti-new.component.scss']
})
export class EcVerticalSubmultiNewComponent implements OnInit {
  ...
  Format = []; // FIN1EURINT-104 : Participation pay off addition in 1Europe
  ...
  showCptyId: boolean;
  commonData: any = []; // FIN1EURINT-104 : Participation pay off addition in 1Europe
  ...
  ngOnInit(): void {
    ...
    try {
      ...
      // FIN1EURINT-520 | Request for TS and KID links disable for LPs not supporting documentation
        ...
        this.creditPriceFlagVerticalSubscription = this.apifunctions.creditpriceFlagCheck.subscribe(async flag => {
          if (flag) {
        ...
        // credit tranche save
        this.creditSaveFlagVerticalSubscription = this.apifunctions.creditSaveFlagCheck.subscribe(async flag => {
          if (flag) {
            await this.validationOnButtonCT();
        ...
        this.rcPriceFlagVerticalSubscription = this.apifunctions.rcpriceFlagCheck.subscribe(async flag => {
        ...
        this.rcSaveFlagVerticalSubscription = this.apifunctions.rcsaveFlagCheck.subscribe(async flag => {
          if (flag) {
            await this.validationOnButton();
            if (this.ShareBasket.length > 0 && this.ErrorMsg === '') {
        ...
        this.ptcPriceFlagVerticalSubscription = this.apifunctions.ptcpriceFlagCheck.subscribe(async flag => {
        ...
        this.ptcSaveFlagVerticalSubscription = this.apifunctions.ptcsaveFlagCheck.subscribe(async flag => {
          if (flag) {
            await this.PTCValidation();
            if (this.ShareBasket.length > 0 && this.ErrorMsg === '') {
	  ...
	  // FIN1EURINT-104 : Participation pay off addition in 1Europe
		...
		// HSBCECCLI-10 : Bulk Pricer - Underlying data discrepancy on clone and toggle | 07-Aug-2023
                ...
                TenorPer: (this.ProductName === 'Accumulator' || this.ProductName == 'Decumulator') ?
                  this.accuTenor + '-' + this.accuTenorType.toString().substring(0,1) : this.creditTenor,
