...
import { Component, ElementRef, EventEmitter, OnInit, Output, ViewChild, Renderer2, QueryList, ContentChildren, ViewChildren  } from '@angular/core';
...
@Component({
  selector: 'app-ec-multi-customstrategy',
  templateUrl: './ec-multi-customstrategy.component.html',
  styleUrls: ['./ec-multi-customstrategy.component.scss']
})
export class EcMultiCustomstrategyComponent implements OnInit {
    ...
    @ViewChild('focusable', { static: false }) namefield: ElementRef;
      ...
      @ViewChildren(EcSubmultirequestErComponent ) childListHorizontal : QueryList<EcSubmultirequestErComponent> ;
      @ViewChildren(EcVerticalSubmultirequestErComponent ) childListVertical : QueryList<EcSubmultirequestErComponent> ;
    ...
    strXMLArr = [];
    strXML = '';
    strXLArr = []; // Bulk Pricer (AC) - Export to excel 
    strXL = ''; // Bulk Pricer (AC) - Export to excel 
    PortFolioName = '';
    successMsg = '';
    portfolio = '';
    portfolioIdArr: any;
    exportFlag = false; // Bulk Pricer (AC) - Export to excel 
    saveFlag = false;
    addFlag: boolean;
    ...
    DisplayProductName = 'Autocallable';
    onBehalfOf = '';
    allBooksData: any = [];
    toggleCheck = false;
      ...
      async ngOnInit() {
        try {
          ...
          this.displayFundingSection = false;
              //$('#loading').show();
              setTimeout(async () => {
                  //   this.portfolioowner = (this.commonfunctions.getLoggedInUserName())[0].UserId;
                  this.portfolioowner = AppConfig.settings.oRes.userID; // Vaibhav B | Added UserID from session | 21-Mar-23
                ...
                //     this.apifunctions.setUserGroup(this.allBooksData[this.allBooksData.findIndex(x => x.BookCode === this.onBehalfOf)].BookName);
                    ...
                    //     //   if (this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'Autocallable') > -1) {
                    //     if (this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'CustomPayoffs') > -1) {
                    //         // this.mappedformatlist = (this.GetClientProdDetailsArr[this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'Autocallable')].CPM_Format).toString().split(',');
                    //         // this.priceBtnActive = this.GetClientProdDetailsArr[this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'Autocallable')].ActiveYN;
                ...
                this.commonfunctions.strXL.subscribe(t => {
                  if (this.exportFlag) {
                      ...
                      if (that.strXLArr.length === that.multiPhoenixArr.length) {
                          this.excelService.exportAsExcelFile(that.strXLArr, 'BulkPricer_Autocall');
            ...
            this.route.params.subscribe
                  (async params => {
                    if (params.RFQ_ID) {
                        ...
                        const index = this.apifunctions.payOffList.findIndex(res=>res.display === true);
                        // Removed onbehalf dependency || PriyaL || 25Apr2022 || Assigned by Pranav D
                         ...
                         // changed by Suvarna P || 27Apr2022 || shares not loading in Autocallable product || assigned by Pranav D 
                           // this.shares = this.apifunctions.BBVALoadShares('EQ',"", index > -1 ? this.apifunctions.payOffList[index].displayName : "CustomPayoffs");
                           this.shares = await this.apifunctions.BBVALoadShares('EQ',"", index > -1 ? this.apifunctions.payOffList[index].prevQuoteProductName : "CustomPayoffs");
                         }
                        ...
                        this.multiPhoenixArr.push({
                            ...
                            ShareBBGRIC2: cloneData.Underlying2 !== undefined ? cloneData.Underlying2[0] : '',
                            ShareBBGRIC3: cloneData.Underlying3 !== undefined ? cloneData.Underlying3[0] : '',
                            ShareBBGRIC4: cloneData.Underlying4 !== undefined ? cloneData.Underlying4[0] : '',
                            ShareBBGRIC5: cloneData.Underlying5 !== undefined ? cloneData.Underlying5[0] : '',
                            ...
                            stkdate: cloneData.stkdate[0],
                               ComputedSettlementPeriodSoftTenor :cloneData.SettShifter[0],
        ...
        try {
            if (this.ErrorMsg === '') {
                ...
                if (this.XMLFlag) {
                      ...
                      this.currentowner = AppConfig.settings.oRes.userID; // Vaibhav B | Added UserID from session | 21-Mar-23
