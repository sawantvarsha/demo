...
Array.prototype.AQsortBy = function (p): Array<any> {
  try {
    if (this !== undefined) {
      if (this.length > 0) {
...
@Component({
  selector: 'app-ec-accumulator',
  templateUrl: './ec-accumulator.component.html',
  styleUrls: ['./ec-accumulator.component.scss'],
})
export class EcAccumulatorComponent implements OnInit {
  temp = '123';
  ...
  @Input() PrevQuoteShowOrderPopUp: any = false;
  ...
  AQPriceSubscription: Subscription;
  TriggerValueArr: any;
  ...
  countryArr = [
    ...
    'Sweden',
    'Switzerland',
    'UK'//Changes by Apurva K || 21-Dec-2023,
  ];
  allSelected = false;
  ...
  isDealer: any = true; // Dealer flag - added by PriyaL on 08Dec2021
  commonData:any = [];
  ...
  async ngOnDestroy(): Promise<void> {
    ...
    if (this.AQPriceSubscription) {
      this.AQPriceSubscription.unsubscribe();
    }
  }
  ...
  ngOnInit() {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        this.docSupportStatus = await this.EcHome.GetDocumentSupportStatus('Accumulator');//Changed by Sandip A || 05-Jan-2023 || To send docs specfic to the entity ID 
        ...
        this.portfolioIdArr = [];
        this.portfolioIdArr = await this.EcHome.BBVAGetPortfolio(
          'Accumulator',
          'Single Pricer'
        );
        ...
        if (
          this.EcHome.validationArr === undefined ||
          this.EcHome.validationArr.length <= 0
        ) {
        ...
        if (
          this.EcHome.rmList === undefined ||
          this.EcHome.rmList.length <= 0
        ) {
        ...
        if (
          this.EcHome.BookingCenter === undefined ||
          this.EcHome.BookingCenter.length <= 0
        ) {
        ...
        // if (this.templateMappingArr && this.templateMappingArr.length > 0) {
          this.commonData = await this.EcHome.GetCommonDataEuroConnect("ACC");
          if (this.commonData && this.commonData.length > 0) {
            this.filldropdownfromcommandata();
          }
          this.defaultvaluesArr = await this.EcHome.GetEntityDefaultValues(
            '',
        ...
        this.Code = () => {
          ...
          if (this.SelectedUnderlyingBasket) {
            ...
            if (
              ...
              this.SelectedUnderlyingBasket.UnderlyingFive.Code
            ) {
        ...
        this.RICCode = () => {
          ...
          if (this.SelectedUnderlyingBasket) {
            if (
              this.SelectedUnderlyingBasket.UnderlyingOne &&
              this.SelectedUnderlyingBasket.UnderlyingOne.RICCode
            ) {
              code = this.SelectedUnderlyingBasket.UnderlyingOne.RICCode;
            }
            if (
              this.SelectedUnderlyingBasket.UnderlyingTwo &&
              this.SelectedUnderlyingBasket.UnderlyingTwo.RICCode
            ) {
              code += ',' + this.SelectedUnderlyingBasket.UnderlyingTwo.RICCode;
            }
            if (
              this.SelectedUnderlyingBasket.UnderlyingThree &&
              this.SelectedUnderlyingBasket.UnderlyingThree.RICCode
            ) {
              code +=
                ',' + this.SelectedUnderlyingBasket.UnderlyingThree.RICCode;
            }
            if (
              this.SelectedUnderlyingBasket.UnderlyingFour &&
              this.SelectedUnderlyingBasket.UnderlyingFour.RICCode
            ) {
              code +=
                ',' + this.SelectedUnderlyingBasket.UnderlyingFour.RICCode;
            }
            if (
              this.SelectedUnderlyingBasket.UnderlyingFive &&
              this.SelectedUnderlyingBasket.UnderlyingFive.RICCode
            ) {
              code +=
                ',' + this.SelectedUnderlyingBasket.UnderlyingFive.RICCode;
            }
        ...
        this.Exchange = () => {
          ...
          if (this.SelectedUnderlyingBasket) {
            ...
            if (
              this.SelectedUnderlyingBasket.UnderlyingFive &&
              this.SelectedUnderlyingBasket.UnderlyingFive.ExchangeCode
            ) {
              exchange +=
                ',' + this.SelectedUnderlyingBasket.UnderlyingFive.ExchangeCode;
            }
        ...
        if (this.Dates) {
          this.todayDate = await this.EcCommon.formatDate(this.Dates.MaturityDate);
        }
        ...
        const res:any = await this.EcHome.GetPriceProviderDetails(
          this.templateMappingArr.length > 0
            ? this.templateMappingArr[0].subTemplate
            : 'Accumulator'
        );
        ...
        const that = this;
        this.AQPriceSubscription = await this.EcCommon.AQSignalRPrices.subscribe(
          (res) => {
            ...
            if (prices && prices.length > 0) {
              ...
              for (let i = 0; i < prices.length; i++) {
                for (let k = 0; k < this.priceProvidersArr.length; k++) {
                  if (prices[i].ppID === this.priceProvidersArr[k].lp) {
                    // handled zero price value with no response status || PriyaL || 09May2022 || price rounding issue raised by Abhilash B.
                    this.priceProvidersArr[k].price =
                      ((prices[i].status + '').toUpperCase() === 'TIMEOUT' &&
                        (this.priceProvidersArr[k].price === '' ||
                          this.priceProvidersArr[k].price === '-'))
                        ? 'Timeout'
                        : ((prices[i].value === null || prices[i].value === '')
                          ? ((prices[i].status + '').toUpperCase() === 'REJECTED'
                            ? 'Rejected'
                            : ((prices[i].status + '').toUpperCase() ===
                              'UNSUPPORTED'
                              ? 'Unsupported'
                              : ((prices[i].status + '').toUpperCase() === 'EXPIRED'
                                ? 'Expired'
                                : '-')))
                          : ((prices[i].value !== null &&
                            prices[i].value !== '' &&
                            ...
                            : (prices[i].value !== null &&
                              prices[i].value !== '' &&
                              (prices[i].status + '').toUpperCase() === 'CANCELLED')
                              ? 'Cancelled'
                    ...
                    this.priceProvidersArr[k].NoteMasterID =
                      prices[i].noteMasterID;
                    ...
                    // quote expired timeout chnages by suvarna P || 15Mar2022 || assigned by Pranav D || start
                    this.priceProvidersArr[k].EP_Quote_Response_At =
                      prices[i].EP_Quote_Response_At;
                    this.priceProvidersArr[k].EP_ValidUntilTime =
                      prices[i].EP_ValidUntilTime;
                  }
                }

                if (
                  this.AllPrices.findIndex(
                    (record) => record.rfq === prices[i].rfqID
                  ) === -1
                ) {
              ...
              var idxBest = this.AllPrices.findIndex(
                (item) => item.bestYN === 'Y'
              );
              if (idxBest > -1) {
                // this.sortedAllPrices = this.AllPrices.AQsortBy('Price');
                this.sortedAllPrices = this.AllPrices.filter(
                  (item) => item.bestYN === 'Y'
                );
              }
              ...
              if (this.sortedAllPrices && this.sortedAllPrices[0]) {
                var idx = this.priceProvidersArr.findIndex(
                  (item) => item.lp == this.sortedAllPrices[0].lp
                );
                ////console.log(idx);
                //if(idx > 0){
              ...
              if (this.sortedAllPrices.length > 0) {
                if (
                  ...
                  this.sortedAllPrices[0].Price !== '-' &&
                  this.sortedAllPrices[0].Price !== 'Timeout' &&
                  this.sortedAllPrices[0].Price > 0 &&
                  this.sendtocptyflag
                ) {
                ...
                if (price !== '') {
                  if (
                    this.elem.nativeElement.querySelector(
                      '#txt' + this.sortedAllPrices[0].solveFor
                    )
                  ) {
                    this.elem.nativeElement
                      .querySelector('#txt' + this.sortedAllPrices[0].solveFor)
                      .classList.add('reply');
                  }
                }
              ...
              if (
                this.sortedAllPrices.length === this.priceProvidersArr.length
              ) {
                that.timeLeft = 0;
              }
            ...
            this.priceProvidersArr.forEach((item) => {
              var idx = this.priceProvidersArr.findIndex(
                (item1) => item1.lp == item.lp
              );
              if (
                item.price === 'Unsupported' ||
                item.price === 'Expired' ||
        ...
        this.route.params.subscribe(async (params) => {
          if (params.RFQ_ID) {
            ...
            preQuoteData1 = await this.EcHome.getPreviousQuoteCloneData(
              params.RFQ_ID,
              'RFQID'
            );
          ...
          if (params.PORTFOLIO_ID) {
            ...
            var saveQuoteData: any;
             await this.EcHome.getRedirectionData(params.PORTFOLIO_ID, 'Accumulator').then((data:any)=>{ 
	       saveQuoteData=data;
            });
          ...
          if (params.Tenor) {
            const tChar = (params.Tenor + '').substring(
              params.Tenor.length - 1
            );
            //console.log(tChar);
            ...
            this.Tenor = (params.Tenor + '').substring(
              0,
              params.Tenor.length - 1
            );
          }
          if (params.Underlyings) {
            ...
            for (var each of list) {
              this.showUnderlying(
                'event',
                SearchunderlyingPipe.prototype.transform(this.shares, each)[0]
              );
            }
          }
        });
          ...
          if (this.viewRFQID) {
            ...
            if (this.EcCommon.isEmptyObject(preQuoteData1)) {
              ...
              if (
                ...
                preQuoteData1.cloneData.DailyNoOfShares[0] =
                  await this.EcCommon.formatNotional(this.Order_Notional);
              }
              ...
              if (
                parseInt(this.Notional.replace(/,/g, '')) <
                parseInt(preQuoteData1.cloneData.MinNotional[0])
              ) {
                this.minNotionalConfirm = true;
              }
            }
  ...
  async reset() {
    try {
      ...
      this.EcCommon.AQPricesObserver.next('');
      this.EcCommon.AQPrices = [];
      await this.EcCommon.setAQReceivedPrices({}, '');
  ...
  setPrevQuoteData1(cloneData, viewOnly) {
    try {
      ...
      if (
        cloneData.MinNotional[0] &&
        cloneData.MinInvst[0] &&
        parseFloat(cloneData.MinNotional[0]) !== 0 &&
        parseFloat(cloneData.MinInvst[0]) !== 0 &&
        cloneData.MinNotional[0] !== '' &&
        cloneData.MinInvst[0] !== ''
      )
        this.minNotional =
          parseFloat(cloneData.MinNotional[0]) <
            parseFloat(cloneData.MinInvst[0])
            ? parseFloat(cloneData.MinInvst[0])
            : parseFloat(cloneData.MinNotional[0]);
      else this.minNotional = 250000.0;
      this.maxNotional =
        cloneData.MaxNotional[0] &&
          parseFloat(cloneData.MaxNotional[0]) !== 0 &&
          cloneData.MaxNotional[0] !== ''
          ? parseFloat(cloneData.MaxNotional[0])
          : 5000000.0;
      ...
      // Azure change
      if (
        cloneData.ShareCode[0] !== undefined &&
        cloneData.ShareCode[0] !== ''
      ) {
        if (
          this.ShareBasket.find((i) => i.Code === cloneData.ShareCode[0]) ===
          undefined
        ) {
          // this.ShareBasket.push({ Code: cloneData.SN1_Custom[0] });
          // // this.SelectedUnderlyingBasket.UnderlyingOne['Code']=cloneData.FirstUnderlyingCode1[0];
          ...
          var index = this.shares.findIndex(
            (shareItem) => shareItem.BloombergCode == cloneData.ShareCode[0]
          );
          if (index >= 0) {
            const shareCode = this.shares[index].Code;
            this.showUnderlying(
      ...
      this.TenorType =
        cloneData.Tenor[0].substr(cloneData.Tenor[0].length - 1, 1) === 'M'
          ? 'Month'
          : 'Year'; // Azure change
      ...
      var idx = this.shares.findIndex(
        (i) =>
          i.BloombergCode == cloneData.ShareCode[0] ||
          i.Code == cloneData.ShareCode[0]
      ); // Azure change
      //console.log(idx);
      ...
      if (viewOnly) {
        ...
        this.rfqID = cloneData.RFQIDcsv[0]
          .trim()
          .substring(0, cloneData.RFQIDcsv[0].length - 2);
        this.noteMasterID = cloneData.Note_Master_Id[0];
        this.NoteMasterID = cloneData.Note_Master_Id[0];
        this.sortedAllPrices = [
          {
            Price:
              cloneData.SolveForValue[0] === '' && this.orderID !== ''
                ? 'Rejected'
                : cloneData.SolveForValue[0],
        ...
        for (
          let i = 0;
          i < cloneData.RFQIDcsv[0].trim().split(',').length - 1;
          i++
        ) {
          this.priceProvidersArr.push({
            ...
            price:
              cloneData.BestValue[0].trim().split(',')[i].trim() === '0' ||
                cloneData.BestValue[0].trim().split(',')[i].trim() === ''
                ? '-'
                : cloneData.BestValue[0].trim().split(',')[i].trim(),
            bestPriceFlag: cloneData.BESTPriceFlag[0]
              ? cloneData.BESTPriceFlag[0].trim().split(',')[i].trim()
              : '',
            //   BESTPriceFlag
            id: cloneData.BBVAIDcsv[0].trim().split(',')[i].trim(),
            timer: '-', // '0'
            TSFlag:
              cloneData.Termsheetcsv[0].trim().split(',')[i].trim() === 'Y'
                ? true
                : false,
            Msg: '',
            KIDFlag:
              cloneData.KIDcsv[0].trim().split(',')[i].trim() === 'Y'
                ? true
                : false,
            TSMsg: '',
        ...
        //console.log(this.priceProvidersArr);
        this.sortedAllPrices = {};
        this.sortedAllPrices = this.priceProvidersArr.filter(
          (item) => item.rfq === this.viewRFQ
        );
        ...
        if (this.sortedAllPrices && this.sortedAllPrices.length > 0) {
          this.sortedAllPrices[0] = Object.keys(this.sortedAllPrices[0]).reduce(
            (object, key) => {
              if (key !== 'price') {
      ...
      if (this.Strike !== '') {
        if (parseFloat(this.Strike) !== 100.0) {
          this.LeverageFlag = true;
        } else {
          this.LeverageFlag = false;
        }
  ...
  async fnGetProdTemplate() {
      ...
      this.templateMappingArr = await this.EcHome.fnGetProdTemplate('Accumulator');
  ...
  async setSaveQuoteData(cloneData, viewOnly) {
    try {
      ...
      if (cloneData.Share.indexOf('##') !== -1) {
        for (
          let i = 0;
          i < cloneData.Share.split('##')[0].split('#').length;
          i++
        ) {
      ...
      let shareCode =
        this.shares[
          this.shares.findIndex(
            (i) => i.BloombergCode === cloneData.ShareBBGRIC1
          )
        ].Code;
      ...
      if (cloneData.LeverageYN === 'Y' || cloneData.LeverageYN === 'Yes') {
        // load template issue || asigned by pranav || 07Mar2022
        this.LeverageYN = 'Yes';
        this.chkLeverage = true;
      } else {
        this.LeverageYN = 'No';
  ...
  showUnderlying(event, item) {
    try {
      ...
      if (this.ShareBasket.length < 1) {
        if (
          this.ShareBasket.find((i) => i.Code === item.BloombergCode) ===
          undefined
        ) {
          this.ShareBasket.push({
            Code: item.BloombergCode,
  ...
  updateShareBasket() {
    try {
      ...
      if (this.SelectedUnderlyingBasketArray[4] === undefined) {
        ...
        this.SelectedUnderlyingBasket.UnderlyingFive =
          this.SelectedUnderlyingBasketArray[4];
      }
    } catch (error) { }
  }
  ...
  async calculateNotional(sharecode: any, ExchangeCode: any, NoofShare: any) {
    try {
      ...
      const accrualdayResp = await this.EcHome.GetNoOfDays(
        ExchangeCode,
        this.Tenor,
        this.TenorType.substr(0, 1),
        this.ddlNoteCcy,
        this.Frequency
      );
      ...
      if (
        ...
        res =
          parseFloat(
            shareInfo['ShareRate'].toString().replace(/,/g, '')
          ) *
          accrualday *
          NoofShare;
        this.Notional = res.toFixed(2);
        this.Notional = this.Notional.toString().replace(
          /\B(?=(\d{3})+(?!\d))/g,
          ','
        );
      }
  ...
  chkLeverageChange() {
    ...
    if (this.chkLeverage) {
      ...
      this.totalShares =
        parseFloat(this.accrualDays.replace(/,/g, '')) *
  ...
  fnGetValidation() {
    try {
      ...
      //console.log("validationarr", this.validationArr);
          ...
          switch (this.validationArr[i].Setting_Name) {
            ...
            case 'EQ_Show_Order_RM':
              if (this.validationArr[i].Config_Value !== '') {
                if (
                  this.validationArr[i].Entity_Id ===
            ...
            case 'EQ_Show_Order_Customer':
              if (this.validationArr[i].Config_Value !== '') {
                if (
                  ...
                  this.EQ_Show_Order_Customer =
                    this.validationArr[i].Config_Value;
                }
              } else {
                this.EQ_Show_Order_Customer =
                  this.validationArr[i].Default_Value;
              }
            ...
            //  Added new entity config to show/hide Order button || Amogh K || 19Apr2022 || Assigned by PranavD
...
async sendordertocounterparty() {
    ...
    for (let i = 0; i < this.commonData.length; i++) {
      switch (this.commonData[i].Field_Name) {
        ...
        case 'NonBestPriceReason':
          this.NonBestPriceReasonArr =
            this.parseCommonDatatoJSONArr('NonBestPriceReason');
      }
    }
  }
  ...
  parseCommonDatatoJSONArr(Field_Name: any) {
    ...
    const index = this.commonData.findIndex(
    ...
    if (displayTxt && displayTxt.length > 0) {
      for (let i = 0; i < displayTxt.length; i++) {
        if (displayTxt[i] !== '') {
          commonDataJSONArr.push({
            Key: displayTxt[i],
            Value: saveTxt[i] === '' ? displayTxt[i] : saveTxt[i],
          });
        }
      }
    }
  ...
  changeAutoFreqOnTenor() {
    try {
      ...
      if (
        ...
        str.substr(str.length - 1, 1).toUpperCase() === 'M' ||
        ...
        if (month === 0 || month === 1) {
          ...
          for (let i = 0; i < this.freqArr.length; i++) {
            // tslint:disable-next-line: radix
            const freq = parseInt(
              this.freqArr[i].substring(0, this.freqArr[i].length - 1)
            );
            if (freq < month) {
        ...
        const autoFreqNum = this.autoFreq
          .toString()
          .substring(0, this.autoFreq.length - 1);
        // if (month < 4) {
        //   this.autoFreq = '1m';
        // } else {
        //   this.autoFreq = '3m';
...
async GetTriggerValue(type: any) {
    ...
    if (type === 'autoTrigger') {
        this.TriggerValueArr = await this.EcHome.GetTriggerValues((this.expshift === 'Custom' ? this.customTenor : this.expshift), this.autoFreq,
    ...
    if (type === 'cpnTrigger') {
        this.TriggerValueArr = await this.EcHome.GetTriggerValues((this.expshift === 'Custom' ? this.customTenor : this.expshift), this.cpnFreq,
            this.cpnTrigger, '', '');
        //console.log(this.TriggerValueArr);
    ...
    if (type === 'fundRate') {
        this.TriggerValueArr = await this.EcHome.GetTriggerValues((this.expshift === 'Custom' ? this.customTenor : this.expshift), this.fundFreq,
            this.fundRate, '', '');
        //console.log(this.TriggerValueArr);
        this.autoTriggerPopup = false;
        this.cpnTriggerPopup = false;
  ...
  setdefaultvalues() {
      ...
      switch (this.defaultvaluesArr[i].Control_Name) {
        ...
        case 'Underlyings':
          ...
          if (
            this.defaultvaluesArr[i].Default_Value &&
            this.defaultvaluesArr[i].Default_Value !== ''
          ) {
            this.defaultvaluesArr[i].Default_Value.split(',').forEach(
              (element) => {
                var index = this.shares.findIndex(
                  (shareItem) => shareItem.BloombergCode == element
                );
                if (index >= 0) {
        ...
        case 'Frequency':
          this.Frequency =
            this.defaultvaluesArr[i].Default_Value.toString().toUpperCase();
          break;
        ...
        case 'GearingLeverage':
          this.chkLeverage =
            this.defaultvaluesArr[i].Default_Value === undefined ||
              this.defaultvaluesArr[i].Default_Value === 'undefined' ||
              this.defaultvaluesArr[i].Default_Value === '' ||
              this.defaultvaluesArr[i].Default_Value === 'false'
              ? false
              : true;
          break;
  ...
  async showOrderDetailsFromPrevQuote() {
    try {
      ...
      if (this.dataItemAccordian.ER_QuoteRequestId) {
        let preQuoteData1: any;
        preQuoteData1 = await this.EcHome.getPreviousQuoteCloneData(
          this.dataItemAccordian.ER_QuoteRequestId,
          'RFQID'
        );
        if (this.EcCommon.isEmptyObject(preQuoteData1)) {
          // this.ErrorMsgTop = 'No data found for this record.';
          ...
          if (
            ...
            this.timeLeftBookOrder =
              new Date(this.dataItemAccordian.EP_ValidUntilTime).getTime() -
              new Date(preQuoteData1.cloneData.Quote_Response_At).getTime();
      ...
      // quote expired timeout chnages by suvarna P || 15Mar2022 || assigned by Pranav D || end
      ...
      this.txtIBPrice =
        this.dataItemAccordian.ER_SolveFor === 'Price(%)'
          ? this.dataItemAccordian.EP_Price
          : this.dataItemAccordian.EP_OfferPrice;
      // this.txtClientPrice = '';
      ...
      this.txtUpfront = parseFloat(
        this.dataItemAccordian.ER_Upfront.toString().replace(/,/g, '')
      ).toFixed(2);
      ...
      if (
        this.dataItemAccordian.ER_UnderlyingCode !== undefined &&
        this.dataItemAccordian.ER_UnderlyingCode !== ''
      ) {
        const sharecsvArr = this.dataItemAccordian.ER_UnderlyingCode.split(',');
        for (let k = 0; k < sharecsvArr.length; k++) {
          var index = this.shares.findIndex(
            (shareItem) =>
              shareItem.Code === sharecsvArr[k] ||
              shareItem.BloombergCode == sharecsvArr[k]
          );
          if (index >= 0) {
            ...
            this.ShareBasket.push({
              Code: this.shares[index].Code,
              LongName: lName,
              Weight: '',
              Exchange: this.exchngCode,
              RICCode: this.shares[index].Code,
            });
            this.setSelectedUnderlyingarray(
              lName,
              Ccy,
              this.shares[index].Code,
              this.exchngCode,
              '',
              '',
              '',
      ...
      this.Tenor = this.dataItemAccordian.ER_RFQTenor.substring(
        0,
        this.dataItemAccordian.ER_RFQTenor.length - 1
      );
      // this.TenorType = this.dataItemAccordian.ER_RFQTenor.substring(this.dataItemAccordian.ER_RFQTenor.length - 1, 1) === 'M' ? 'Month' : 'Year';
      this.TenorType =
        this.dataItemAccordian.ER_RFQTenor.substring(
          this.dataItemAccordian.ER_RFQTenor.length - 1
        ) === 'M'
          ? 'Month'
          : 'Year';
  ...
  hideOrderDetails() {
    ...
    // quote expired timeout chnages by suvarna P || 15Mar2022 || assigned by Pranav D
  ...
  ChangeIndex(_e) {
    try {
      if (_e.keyCode === 32 && this.ShareName == undefined) {
        _e.preventDefault();
...
async txtTenorDateChange(type: any) {
  try {
      ...
      if (type === 'Payment') {
          ...
          this.expdate = await this.EcCommon.formatDate(this.Dates.MaturityDate);
          // this.datepipe.transform(this.Dates.MaturityDate, 'yyyy-MM-dd');
      }
      if (type === 'Expiry') {
...
cloneSinglePricer() {
  ...
  async RequestTermsheet() {
    try {
        ...
        this.ErrorMsgTop = '';
        // Removed onbehalf dependency || PriyaL || 25Apr2022 || Assigned by Pranav D
        // const errorMsg = this.apifunctions.termsheetSender(this.orderID, this.onBehalfOf, this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'Accumulator', this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'Accumulator', 'TS');
        const errorMsg = await this.EcHome.termsheetSender(this.orderID, '', this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'Accumulator', this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'Accumulator', 'TS');
        if (errorMsg === '') {
            ...
            that.TSInterval = setInterval(async() => {
                if (TSTimeLeft > 0) {
                    const res = await this.EcHome.ViewTermsheet(that.sortedAllPrices[0].rfq, 'IndicativeTermsheet');
...
async RequestKIDonView() {
  try {
      ...
      this.ErrorMsgTop = '';
      // Removed onbehalf dependency || PriyaL || 25Apr2022 || Assigned by Pranav D
      // const errorMsg = this.apifunctions.termsheetSender(this.sortedAllPrices[0].id, this.onBehalfOf, 'AutocallablePhoenix', 'AutocallablePhoenix', 'KID');
      const errorMsg =  await this.EcHome.termsheetSender(this.sortedAllPrices[0].id, '', 'AutocallablePhoenix', 'AutocallablePhoenix', 'KID');
      ...
      if (errorMsg === '') {
          ...
          that.TSInterval = setInterval(async () => {
              if (TSTimeLeft > 0) {
                  const res = await this.EcHome.ViewTermsheet(that.sortedAllPrices[0].rfq, 'Kid');//Apurva K|| FIN1EURINT-385|| 23-May-2023
                  if (res !== null && res !== undefined) {
                      if (res.Status.toString().toUpperCase() === 'SUCCESS') {
...
async ViewKIDonView() {
  ...
  PriceValidation(e, Pricestr: string) {
    try {
      ...
      switch (Pricestr) {
        case 'Strike':
          if (parseFloat(target.value) <= parseFloat(this.minStrike)) {
            this.ErrorMsgTop =
              'Strike should be greater than ' + this.minStrike + '%.';
        ...
        case 'CouponTrigger':
          if (target.value === '') {
            this.ErrorMsgTop =
              'Coupon trigger should be between ' +
              this.minCoupon +
              '% and ' +
              this.maxCoupon +
              '%.';
            ...
            if (
              parseFloat(target.value) <= parseFloat(this.minCoupon) ||
            ...
            for (let i = 0; i < cpnArr.length; i++) {
              if (
                parseFloat(cpnArr[i]) <= parseFloat(this.minCoupon) ||
        ...
        case 'BarrierLevel':
          if (parseFloat(target.value) <= parseFloat(this.minBarrier)) {
            this.ErrorMsgTop =
              'Barrier level should be greater than ' + this.minBarrier + '%.';
            target.classList.add('error');
          }
          break;
        case 'Stepdown':
          if (parseFloat(target.value) <= parseFloat(this.minStepdown)) {
            this.ErrorMsgTop =
              'Stepdown should be greater than ' + this.minStepdown + '%.';
            target.classList.add('error');
          }
          break;
        ...
        case 'AutoTrigger':
          if (target.value === '') {
            this.ErrorMsgTop =
              'Autocall trigger should be between ' +
              this.minTrigger +
              '% and ' +
              this.maxTrigger +
              '%.';
            ...
            if (target.value.indexOf('/') < 0) {
              if (
                parseFloat(target.value) <= parseFloat(this.minTrigger) ||
      ...
      $(
        '<div class="error-input"></div><div class="validate-popup active"><span> ' +
        this.ErrorMsgTop +
        '</span></div>'
      ).insertAfter('.error');
      $('.validate-popup').delay(5000).fadeOut('slow');
...
async txtTenorChange(e, type: any) {
  ...
  AQCalculator() {
    if (this.UnderlyingPrice !== '' && this.StrikePer !== '') {
      this.StrikePrice = (
        (parseFloat(this.UnderlyingPrice) * parseFloat(this.StrikePer)) /
        100
      ).toFixed(2);
      ...
      if (this.NotionalAmount !== '' && this.calcDayCount !== '') {
        if (this.NotionalAmount.includes(',')) {
          this.NoShares = (
            parseFloat(this.NotionalAmount.replace(/,/g, '')) /
            (parseFloat(this.StrikePrice) * parseFloat(this.calcDayCount))
          ).toFixed(2);
        } else {
          this.NoShares = (
            parseFloat(this.NotionalAmount) /
            (parseFloat(this.StrikePrice) * parseFloat(this.calcDayCount))
          ).toFixed(2);
        }
      }
      if (this.NotionalAmount !== '') {
        this.NotionalAmount = this.NotionalAmount.toString().replace(
          /\B(?=(\d{3})+(?!\d))/g,
          ','
        );
        this.NoShares = this.NoShares.toString().replace(
          /\B(?=(\d{3})+(?!\d))/g,
          ','
        );
      }
  ...
  async checkValidNotional(e, type: any) {
    try {
      ...
      if (NotionalData.ErrorMsg === '') {
        ...
        if (type === 'NoOfShare') {
          this.NoOfShare = parseFloat(
            NotionalData.Notional.replace(/,/g, '')
          ).toFixed(0);
        ...
        if (type === 'Notional') {
          $(
            ...
            this.ErrorMsgTop +
            '</span></div>'
          ).insertAfter('#txtnotional');
          $('.validate-popup').delay(5000).fadeOut('slow');
        }
        if (type === 'NotionalAmt') {
          document
            .getElementById('txtNotionalAmount')
  ...
  changeTenor() {
    try {
      ...
      if (
        this.Tenor == 0 ||
        ((this.Tenor > 60 ||
          this.Tenor > this.tenorMaxMonth ||
          this.Tenor < this.tenorMinMonth) &&
          this.TenorType === 'Month') ||
        ((this.Tenor > 5 ||
          this.Tenor * 12 > this.tenorMaxMonth ||
          this.Tenor * 12 < this.tenorMinMonth) &&
          this.TenorType === 'Year')
      ) {
        // Changed by Amogh K | 29Apr2022 | tenor validation modified | assigned by Pranav D
        this.ErrorMsgTop =
          'Please enter tenor between ' +
          this.tenorMinMonth +
          ' to ' +
          this.tenorMaxMonth +
          ' months';
  ...
  ChangeKO(_e) {
    try {
      ...
      if (parseFloat(this.KO) < 102) {
        this.ErrorMsgTop = 'KO % should be greater than or equal to 102';
      }
    } catch (error) {
      ////console.log("Error:", error);
    }
  ...
  Guaranteechange() {
    ...
    if (this.Frequency === 'BIWEEKLY') {
      if (
        parseFloat(this.Guarantee) / 2 > this.Tenor / 3 &&
        this.TenorType === 'Month'
      ) {
        this.ErrorMsgTop =
        ...
        $(
          '<div style="margin-left:5px;" class="error-input"></div><div class="validate-popup active"><span> ' +
          this.ErrorMsgTop +
          '</span></div>'
        ).insertAfter('#ddlGuarantee');
        $('.validate-popup').delay(5000).fadeOut('slow');
      }
    } else {
      if (
    ...
    if (this.Frequency === 'BIWEEKLY') {
      if (
        parseFloat(this.Guarantee) / 2 > (this.Tenor * 12) / 3 &&
        this.TenorType === 'Year'
      ) {
        this.ErrorMsgTop =
          'Guarantee period should be less than or equal to 1/3 of tenor.';
      ...
      if (
        parseFloat(this.Guarantee) > (this.Tenor * 12) / 3 &&
        this.TenorType === 'Year'
      ) {
        this.ErrorMsgTop =
          'Guarantee period should be less than or equal to 1/3 of tenor.';
  ...
  validationOnButton() {
    try {
      ...
      if (
        this.templateMappingArr === undefined ||
        this.templateMappingArr.length <= 0
      ) {
        this.ErrorMsgTop =
          'Service unavailable. Please reload the application and try again.';
        return false;
      } else {
        this.ErrorMsgTop = '';
      }
      ...
      if (
        this.SolveForvalue !== 'Strike' &&
        parseFloat(this.Strike) <= parseFloat(this.minStrike)
      ) {
        this.ErrorMsgTop =
          'Strike should be greater than ' + this.minStrike + '%.';
      ...
      if (this.SolveForvalue !== 'Strike' && parseFloat(this.Strike) > 100) {
        this.ErrorMsgTop = 'Strike % should be less than 100.';
        document.getElementById('txtStrike').classList.add('error');
        $(
          '<div class="error-input"></div><div class="validate-popup active"><span> ' +
          this.ErrorMsgTop +
      ...
      if (this.Guarantee === '') {
        // Guarentee Periods validation changes || asigned by pranav || 07Mar2022
      ...
      if (this.NoOfShare == 0 || this.NoOfShare === '') {
        ...
        $(
          ...
          this.ErrorMsgTop +
          '</span></div>'
        ).insertAfter('#idNoOfShare');
        $('.validate-popup').delay(5000).fadeOut('slow');
      }

      // Added by Amogh K | 13 Apr 2022 | added upfront validation
      if (
        this.SolveForvalue !== 'Upfront' &&
        (this.upfront === '' ||
          parseFloat(this.upfront) <= 0 ||
          parseFloat(this.upfront) > 100)
      ) {
  ...
  async AQPrice(LPVal, NMID, repriceFlg) {
    try {
      var LP = '';
      this.priceBtnActive =  'N' //Added by ApurvaK||04-Apr-2023|| Remove multiple clicks pn Price button
      if (LPVal == 'All') {
        let isPriceFlgChk = this.priceProvidersArr.findIndex(
          (item) => item.priceChkFlg == true
        );
        if (isPriceFlgChk == -1) {
          this.priceProvidersArr.forEach(item => {
          ...
          if (lastCharater == ',') LP = LP.substring(0, LP.length - 1);
          // }
        }
        //  LP = '';
      } else {
        ...
        // var idx = this.priceProvidersArr.findIndex(
        //   (item1) => item1.lp == LPVal
        // );
        // if (idx > -1) {
        //   this.priceProvidersArr[idx].loadFlag = true // Added by Jyoti S || 02-May-2023
        //   this.priceProvidersArr[idx].timer = this.startCountDown(
        //     this.defaultTimeout,
        //     idx
        //   );
        // }
              ...
              that.interval = setInterval(async () => {
                ...
                if (that.timeLeft > 0) {
                  ...
                  if (!quoteResponseCheck) {
                    quoteResponseCheck = await that.AQPriceResponse(that.PPDetails);
                  }
      ...
      //             if (rfqPPID[k].toString().includes(':')) {
      //               const splitRFQID = rfqPPID[k].split(':');
      //               const index = that.priceProvidersArr.findIndex(
      //                 (res) => res.lp === splitRFQID[0].toString().trim()
      //               );
      //               if (index > -1) {
      //                 that.priceProvidersArr[index].rfq = splitRFQID[1];
      //               }
      ...
      //           if (that.timeLeft > 0) {
      //             that.timeoutMsg = '';
      //             that.AQPriceResponse(that.PPDetails);
 ...
 async portfolioChange() {
    ...
    if (this.portfolio !== '') {
        ...
        setTimeout(async () => {
             ...
             var saveQuoteData:any;
             await this.EcHome.getRedirectionData(this.portfolio,'Accumulator').then((data:any)=>{
  ...
  try {
      ...
      this.successMsg = ''
      if (this.portfolio !== '') {
          const res = await this.EcHome.BBVADeletePortfolio(this.portfolio, 'Accumulator');
          if (res) {
              this.saveFlag = true;
              ...
              this.portfolioIdArr = [];
              await this.EcHome.BBVAGetPortfolio('Accumulator', 'Single Pricer').then((data:any)=>{this.portfolioIdArr=data});
              this.portfolioIdArr.splice(0, 0, {
                  AccessDetail: "ALL",
                  Created_At: "",
                  P_ID: "",
  ...
  async AQPriceResponse(PPDetails) {
    try {
      ...
      return await this.http.post(webMethod, parameters).toPromise().then(async (data: any) => {
            that.Prices = data;
            await that.EcCommon.setAQReceivedPrices(that.Prices, 1);
          ...
          this.priceProvidersArr.forEach(item=>{
            ...
            if (!this.priceClickFlag) {
                ...
                if (this.sendtocptyflag) {  
                    // cleared Pricer Provider Array on reprice action - added by PriyaL.on 25Feb2022 - assigned by Pranav D.
                    this.priceProvidersArr = [];
                    const res:any = await this.EcHome.GetPriceProviderDetails(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'Accumulator');
                    for (let i = 0; i < res.length; i++) {
                        if (res[i] === res[i]) {
                    ...
                    // this.AQPrice(LP,this.NMID,'Y');
                    this.AQPrice(LP, this.viewRFQID, 'Y');
                }
                else {
                    this.AQPrice(LP, "0", 'N');
                }
  ...
  generateXML() {
    try {
      //console.log(this.templateMappingArr);
      // const d:any = this.EcHome.BBVAGetDates(
      //   this.ddlNoteCcy,
      //   (
      //     (this.expshift === 'Custom' ? this.customTenor : this.expshift) + ''
      //   ).toUpperCase(),
          ...
          switch (this.templateMappingArr[i].email_Header) {
            ...
            case 'Tenor':
              xmlstr +=
                ...
                '>' +
                (this.Tenor +
                  this.TenorType.toString().substr(0, 1).toUpperCase()) +
                '</' +
                this.templateMappingArr[i].template_Field_Name +
                '>';
              break;
            ...
            case 'underlying1':
              xmlstr +=
                ...
                this.templateMappingArr[i].template_Field_Name +
                '>' +
                (this.ShareBasket[0].Code === undefined
                  ? ''
                  : this.ShareBasket[0].Code) +
                '</' +
                this.templateMappingArr[i].template_Field_Name +
                '>';
              break;
            case 'exchange1':
              xmlstr +=
                ...
                this.templateMappingArr[i].template_Field_Name +
                '>' +
                (this.ShareBasket[0].Exchange === undefined
                  ? ''
                  : this.ShareBasket[0].Exchange) +
                '</' +
                this.templateMappingArr[i].template_Field_Name +
                '>';
              break;
...
SelectedTab(Tab: any) {
  ...
  // this.commonfunctions.setAccord(8, 'Accumulator')
}
  lauchProduct(priceRecord) {
    ...
    this.selectRecord['Template_Code'] = 'Accumulator';
  ...
  async calcAllocataionNotionalForLimitOrder(sharecode: any, ExchangeCode: any, NoofShare: any) {
    ...
    try {
        ...
        // const shareInfo = this.apifunctions.GetSharesInfo(sharecode)
        const shareInfo = await this.EcHome.GetShareRate(sharecode, this.ddlNoteCcy)
        const accrualday:any = await this.EcHome.GetNoOfDays(ExchangeCode, this.Tenor, this.TenorType.substr(0, 1), this.ddlNoteCcy, this.Frequency);
        this.accrualDays = accrualday;
        // changes by Suvarna P || 22Apr2022 || GetSharesInfo chnaged to GetShareRate || assigned by Pranav D
        // if (shareInfo && shareInfo.length > 0 && accrualday !== '') {
        if (shareInfo && shareInfo !== '' && accrualday !== '') {
            ...
            if (this.txtOrderType === 'Limit') {
                ...
                // res = shareInfo[0].Spot * accrualday * NoofShare;
                res = parseFloat(shareInfo.toString().replace(/,/g, '')) * accrualday * NoofShare;
            }
  ...
  async checkValidAllocateNotional(e, rowindex) {
    try {
      ...
      if (NotionalData.ErrorMsg === '') {
        // this.Notional = NotionalData.Notional;
        // this.allocatedNotionalArr[rowindex] = parseFloat(NotionalData.Notional).toFixed(0);
        // e.target.value = this.Notional;
  ...
  }// Function added by Jyoti S || 14-Apr-2023

  async currencyChange() {
    try {
        this.reset();
        this.checkNotionalRes = await this.EcHome.BBVACheckNotional(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'Accumulator', this.ddlNoteCcy);
  ...
  async chkAll(e) {
    ...
    if (target.checked) {
      this.allocatedNotional = '0';
      this.tblAllocation.forEach((_elment) => {
        ////console.log(element);
      ...
      this.tblAllocation.forEach((_element) => {
        ////console.log(element);
  ...
  async chkAllocation(e, index: any) {
    ...
    this.ErrorMsg1 = '';
    const target = await this.EcHome.GetEventTarget(e);
    //console.log(target.id, e, target.checked);
    if (!(target.checked)) {
...
freqChange() {
  ...
  try {
      ...
      // const res: any = this.apifunctions.GenerateDocument(this.priceProvidersArr[index].NoteMasterID); 
...
async createPool() {
  ...
  // if(!this.commonfunctions.fnCompareNotional(this.txtnotional,this.allocatedNotional)){
  ...
  const res: any = await this.EcHome.RedirectOrderValidationChecks("", this.txtOrderType, this.txtlimitLevel.toString(), this.txtClietYield,
      parseFloat(this.txtUpfront) > 1 ? parseFloat(this.txtUpfront).toString() : (parseFloat(this.txtUpfront) * 100).toString(), this.SolveForvalue, this.Issuer, this.issuePrice, "",
      (this.templateMappingArr !== undefined && this.templateMappingArr.length > 0) ? this.templateMappingArr[0].template : 'ACC', estAllocatedNotionalSum.toString(), this.Code(), this.ddlNoteCcy, this.ddlNoteCcy, this.Tenor.toString());
...
confirmValidation1() {
  ...
  var rtnFlg = true;
  const that = this;
  this.tblAllocation.forEach(function (_element) {
      ////console.log((<HTMLInputElement>document.getElementById("Notional" + i)).value);
      //console.log(i);
      //console.log((<HTMLInputElement>document.getElementById("checkbox" + i)).checked)
      //console.log((<HTMLInputElement>document.getElementById("Notional" + i)).value);
...
priceValidation(priceValue: any, pricestr: string) {
  try {
      ...
      if (pricestr !== 'Strike (%)' && (priceValue === '' || parseFloat(priceValue) <= 0 || parseFloat(priceValue) >= 100)) {
          ...
          this.ErrorMsgTop = pricestr + ' should be greater than 0 and less than 100';
      }
  }
  catch (error) {
      ////console.log("Error:", error);
  ...
  try {
    ...
    this.reset(); //Added to fix timeout bug on Save New variant call while pricing|| 13-Jun-2023
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
          ...
          var res :any;
          // debugger;
           await this.EcHome.BBVASaveQuotes('Single Pricer', strXml, this.portfolioName, '', 'Accumulator', AppConfig.settings.oRes.userID).then((data:any)=>{res=data});
          if (res) {
              if (res.errorMessage === '') {
              ...
              // handle error message while saving duplicate records || PriyaL || 04May2022 || issue reported by AbeerJ
              else {
...
async updatePortfolio() : Promise<any> {
  try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
          ...
          this.reqSuccessMsg = '';
          const strXml = '<Details>' +await this.generateSaveXML() + '</Details>';
          const res = await this.EcHome.BBVASaveQuotes('Single Pricer', strXml, this.portfolioName, this.portfolio, 'Accumulator', AppConfig.settings.oRes.userID);
          if (res) {
              if (res["errorMessage"] === '') {
                  ...
                  // this.successMsg = 'Portfolio ID ' + res.PortFolioID + ' saved successfully.';
                  this.portfolioIdArr = [];
                  this.portfolioIdArr = await this.EcHome.BBVAGetPortfolio('Accumulator', 'Single Pricer');
                  this.portfolioIdArr.splice(0, 0, {
                      AccessDetail: "ALL",
                      Created_At: "",
                      P_ID: "",
...
async scheduleSend() {
  try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
          ...
          const xmlstr = this.generateXML();
          // Removed onbehalf dependency || PriyaL || 25Apr2022 || Assigned by Pranav D
          // const res = this.apifunctions.SchedulePrice(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'Accumulator', xmlstr,
          ...
          const res = await this.EcHome.SchedulePrice(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'Accumulator', xmlstr,
