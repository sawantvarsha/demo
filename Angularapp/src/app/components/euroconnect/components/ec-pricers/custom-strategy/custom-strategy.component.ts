...
export class CustomStrategyComponent implements OnInit {
 ...
 ngOnInit() {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        this.docSupportStatus = await this.apifunctions.GetDocumentSupportStatus('CustomPayoffs');//Changed by Sandip A || 05-Jan-2023 || To send docs specfic to the entity ID 
        this.LanguageArr = [];
       ...
       // this.shares = await this.apifunctions.BBVALoadShares('EQ,IDX', "", "EQC_Europe");
        ...
        this.commonData = await this.apifunctions.GetCommonDataEuroConnect("CustomPayoffs");
        ...
        this.defaultvaluesArr = await this.apifunctions.GetEntityDefaultValues("", AppConfig.settings.oRes.userID, 'CustomPayoffs');
        ...
        //     if (this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'CustomPayoffs') > -1) {
        //       this.mappedformatlist = (this.GetClientProdDetailsArr[this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'CustomPayoffs')].CPM_Format).toString().split(',');
        //       this.priceBtnActive = this.GetClientProdDetailsArr[this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'CustomPayoffs')].ActiveYN;
        ...
        this.strikeType = 'Single Observation';
       ...
       //To be check
        ...
        const res:any = await this.apifunctions.GetPriceProviderDetails(this.templateMappingArr?.length > 0 ? this.templateMappingArr[0].template : 'CustomPayoffs');
        ...
        this.route.params.subscribe
          (async params => {
            ...
            if (params.PORTFOLIO_ID) {
              ...
              const saveQuoteData:any = await this.apifunctions.getRedirectionData(params.PORTFOLIO_ID,'CustomPayoffs');
  ...
  async fnGetProdTemplate() {
    try {
      ...
      this.templateMappingArr = await this.apifunctions.fnGetProdTemplate('Custom Strategy');
        ...
        for (const i in this.templateMappingArr) {
          switch (this.templateMappingArr[i].email_Header) {
            ...
            case 'LN5': xmlstr += '<' + this.templateMappingArr[i].template_Field_Name + '>'
              ...
              + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              break;
            case 'PayoutDesc': xmlstr += '<' + this.templateMappingArr[i].template_Field_Name + '>'
              + (this.payoutDesc)
              + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              break;
 ...
 async ERPrice(LPVal, NMID, repriceFlg) {
    try {
      ...
      const parameters = [{
        ...
        subTemplateCode: this.templateMappingArr?.length > 0 ? this.templateMappingArr[0].subTemplate : 'CustomPayoffs',
 ...
 async Save() {
    try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
        ...
        var res:any;
        await this.apifunctions.BBVASaveQuotes('Single Pricer', strXml, this.portfolioName, '', 'CustomPayoffs', AppConfig.settings.oRes.userID).then((data:any)=>{res = data});
        if (res) {
          if (res.errorMessage === '') {
            ...
            this.portfolioIdArr = [];
           await this.apifunctions.BBVAGetPortfolio('CustomPayoffs', 'Single Pricer').then((data:any)=>{this.portfolioIdArr = data});
            this.portfolioIdArr.splice(0, 0, {
              AccessDetail: "ALL",
              Created_At: "",
              P_ID: "",
  ...
  async generateSaveXML() {
	...
	this.commonfunctions.generateFlexiXml({});
      ...
      this.tempXML += '<Record>' +
        ...
        '<TenorPer></TenorPer>' +
        '<TenorType></TenorType>' +
        '<StrikeType>' + this.strikeType + '</StrikeType>' +
        ...
        '<SubTemplate>' + (this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'CustomPayoffs') + '</SubTemplate>' +
  ...
  async checkNotional(e) {
    try {
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
        ...
        this.checkNotionalRes = await this.apifunctions.BBVACheckNotional(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'CustomPayoffs', this.ddlNoteCcy);
  ...
  async RequestTermsheet() {
    try {
      ...
      //const errorMsg: any = this.apifunctions.termsheetSender(this.orderID, '', this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'CustomPayoffs', this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'CustomPayoffs', 'TS');
      const res: any = await this.apifunctions.termsheetSender(this.sortedAllPrices[0].lp, this.sortedAllPrices[0].rfq, 'CustomPayoffs', this.language, this.country);
...
//Changed by Jyoti S || 31-May-2023 || END
  ...
  async RequestKIDonView() {
    try {
      ...
      const res: any = await this.apifunctions.KIDSender(this.sortedAllPrices[0].lp, this.sortedAllPrices[0].rfq, 'CustomPayoffs', this.language, this.country);
...
// Modified function by Adil for Custom barrier|| HSBCECCLI-42 || 17-08-2023
  ...
  async scheduleSend() {
    try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
        ...
        const res = this.apifunctions.SchedulePrice(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'CustomPayoffs', xmlstr,
  ...
  async RequestKID(index: any) {
    try {
      ...
      //Modified by Sudarshan P|| 20-Apr-2023
      const res: any = await this.apifunctions.KIDSender(this.priceProvidersArr[index].lp, this.priceProvidersArr[index].rfq, 'CustomPayoffs', this.language, this.country);
 ...
 async RequestTS(index: any) {
    try {
      ...
      //const errorMsg: any = this.apifunctions.termsheetSender(this.priceProvidersArr[index].id, '', this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'CustomPayoffs',
       // this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'CustomPayoffs', 'TS');
        const res: any = await this.apifunctions.termsheetSender(this.priceProvidersArr[index].lp, this.priceProvidersArr[index].rfq, 'CustomPayoffs', this.language, this.country);
        //Changes by Apurva K|| 18-May-2023 || for Awaited response in request TS
      if ((res?.status.toLowerCase() === 'success' || res?.status.toLowerCase() === 'awaited') && (this.priceProvidersArr[index].price !== "-")) {
  ...
  portfolioChange() {
    ...
    if (this.portfolio !== '') {
      ...
      setTimeout(async () => {
        ...
        const saveQuoteData: any = await this.apifunctions.getRedirectionData(this.portfolio,'CustomPayoffs');
        if (saveQuoteData.length === 0) {
          this.ErrorMsgTop = 'No data found for this record.';
        } else {
...
//Changed by Jyoti S || FIN1EURINT-600 || 30-Aug-2023 || END
  ...
  async DeleteTemplate() {
    try {
      ...
      if (this.portfolio !== '') {
        const res = await this.apifunctions.BBVADeletePortfolio(this.portfolio, 'CustomPayoffs');
        if (res) {
          this.saveFlag = true;
          ...
          this.portfolioIdArr = [];
          this.portfolioIdArr = await this.apifunctions.BBVAGetPortfolio('CustomPayoffs', 'Single Pricer');
          this.portfolioIdArr.splice(0, 0, {
            AccessDetail: "ALL",
            Created_At: "",
            P_ID: "",
  ...
  SelectedTab(Tab: any) {
    ...
    // this.commonfunctions.setAccord(5, 'CustomPayoffs')
  }
  selectRecord = {};
  prevQuoteLaunchPopUp = false;
  lauchProduct(priceRecord) {
    console.log("Price in custom strategy", priceRecord, this.priceProvidersArr)
    ...
    this.selectRecord['Template_Code'] = 'CustomPayoffs';
  ...
  async updatePortfolio() {
    try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
        ...
        const strXml = '<Details>' + await this.generateSaveXML() + '</Details>';
        var res :any;
         await this.apifunctions.BBVASaveQuotes('Single Pricer', strXml, this.portfolioName, this.portfolio, 'CustomPayoffs', AppConfig.settings.oRes.userID).then((data:any)=>{res=data});
        if (res) {
          if (res.errorMessage === '') {
