...
@Component({
  selector: 'app-yield-enhancement',
  templateUrl: './yield-enhancement.component.html',
  styleUrls: ['./yield-enhancement.component.scss']
})
export class YieldEnhancementComponent implements OnInit, OnDestroy {
  ...
  clearFlag: boolean;
  format: any;
  ordertype:any; // Start || Avinash G || 12-Apr-2024 || HSBCECCLI-120 || Order Type not getting saved in the App table via UI XML
  ...
  YEPriceSubscription: Subscription;
  TriggerValueArr: any;
 ...
 // { 'Key': 'Italy', 'Value': 'Italy' },
  ...
  //<Added by Apurva K|| 23-Mar-2023||HSBCECCLI-32>
...
//</Added by Apurva K|| 23-Mar-2023||HSBCECCLI-32>
  ...
  ngOnDestroy(): void {
    ...
    if (this.YEPriceSubscription) {
      this.YEPriceSubscription.unsubscribe();
    }
  }
    ...
    try {
      ...
      this.portfolioIdArr = await this.apifunctions.BBVAGetPortfolio('YieldEnhancement', 'Single Pricer');
        ...
        this.commonData = await this.apifunctions.GetCommonDataEuroConnect("YieldEnhancement");
          ...
          this.defaultvaluesArr = await this.apifunctions.GetEntityDefaultValues("", AppConfig.settings.oRes.userID, 'YieldEnhancement');
        ...
        const res:any = await this.apifunctions.GetPriceProviderDetails(this.templateMappingArr?.length > 0 ? this.templateMappingArr[0].template : 'YieldEnhancement');
        ...
        for (let i = 0; i < res.length; i++) {
          ...
          this.priceProvidersArr.push({
            ...
            MatchBtnDisable: false, //Added by Apurva K|| 22-Jan-2023 || HSBCECCLI-32
          });
        ...
        const that = this;
        this.YEPriceSubscription = await this.commonfunctions.YESignalRPrices.subscribe(res => {
          const prices = res.price;
          // if (this.commonfunctions.isEmptyObject(prices)) {
          //   // //console.log('blank');
          //   this.sortedAllPrices = [];
          //   this.AllPrices = [];
          //   this.sortedAllPrices = [];
          ...
          //   // this.commonfunctions.setPHXReceivedPrices({}, '');
          ...
          this.priceProvidersArr.forEach(item => {
            ...
            if (item.price === 'Unsupported' || item.price === 'Expired' || item.price === 'Cancelled') {
              ...
              //this.priceProvidersArr[idx].MatchBtnDisable=true; //Added by Apurva K || 22-Jan-2023|| HSBCECCLI-32
            }
          });
        });
        this.route.params.subscribe
          (async params => {
            ...
            if (params.PORTFOLIO_ID) {
              ...
              const saveQuoteData:any = await this.apifunctions.getRedirectionData(params.PORTFOLIO_ID,'YieldEnhancement'); //Puja W | Added product parameter 
        ...
        for (let i = 0; i < this.validationArr.length; i++) {
          switch (this.validationArr[i].Setting_Name) {
            ...
            case 'EQ_Show_KID_Button':
              ...
              break;
           //Start - Added by Sudarshan P || 14-Aug-2023|| HSBCECCLI-32||23-Mar-2024
          ...
          case 'EQ_PriceMatch_LPName':
            ...
            break;
        //End - Added by Sudarshan P || 14-Aug-2023|| HSBCECCLI-32||23-Mar-2024
          }
        }
      }
    } catch (error) {
  ...
  async Price() {
    try {
      ...
      // if (this.priipsYN === 'Yes') {
      //   //console.log(this.country);
      //   if (this.country === '' || this.country.length === 0) {

      //     this.ErrorMsgTop = "Please select the country.";
      //     document.getElementById('ddlcountry').classList.add('error');
      //     $('<div class="error-input"></div><div class="validate-popup active"><span> ' + this.ErrorMsgTop + '</span></div>').insertAfter("#ddlcountry")
      //     $('.validate-popup').delay(5000).fadeOut('slow');
      //     return false;
      //   }
      //   if (this.termsheetType === '') {
      //     this.ErrorMsgTop = "Please select the termsheet.";
      //     document.getElementById('ddltermsheet').classList.add('error');
      //     $('<div class="error-input"></div><div class="validate-popup active"><span> ' + this.ErrorMsgTop + '</span></div>').insertAfter("#ddltermsheet")
      //     $('.validate-popup').delay(5000).fadeOut('slow');
      //     return false;
      //   }
      //   //console.log(this.language);
      //   if (this.language === '' || this.language.length === 0) {
      //     this.ErrorMsgTop = "Please select the language.";
      //     document.getElementById('ddllanguage').classList.add('error');
      //     $('<div class="error-input"></div><div class="validate-popup active"><span> ' + this.ErrorMsgTop + '</span></div>').insertAfter("#ddllanguage")
      //     $('.validate-popup').delay(5000).fadeOut('slow');
      //     return false;
      //   }
      // }
  ...
  async PriceLP(LP) {
    try {
      ...
      this.priceProvidersArr = [];
      const res = this.apifunctions.GetPriceProviderDetails(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].template : 'YieldEnhancement');
      for (let i = 0; i < res.length; i++) {
        if (res[i] === res[i]) {
        ...
        for (const i in this.templateMappingArr) {
          switch (this.templateMappingArr[i].email_Header) {
            ...
            case 'loginUser': xmlstr += '<' + this.templateMappingArr[i].template_Field_Name + '>'
              + AppConfig.settings.oRes.userName  //Added by Apurva K
              //+ (this.commonfunctions.getLoggedInUserName())[0].UserId
              + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              break;
  ...
  // Added new mappings || PriyaL || 06Apr2022 || Assigned by PranavD
  generateXML() {
    try {
      ...
      if (this.templateMappingArr && this.templateMappingArr.length > 0) {
        ...
        for (const i in this.templateMappingArr) {
          switch (this.templateMappingArr[i].email_Header) {
            ...
            case 'loginUser': xmlstr += '<' + this.templateMappingArr[i].template_Field_Name + '>'
              + AppConfig.settings.oRes.homeEntityID  //Added by Apurva K
              //+ (this.commonfunctions.getLoggedInUserName())[0].UserId
              + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              break;
            ...
            // Added new field to pass PutGearingYN || PriyaL || 12Apr2022 || Assigned by Pranav D.
            case 'LeverageYN': xmlstr += '<' + this.templateMappingArr[i].template_Field_Name + '>'
              + this.LeverageYN
              + '</' + this.templateMappingArr[i].template_Field_Name + '>';
              break;
  ...
  async YEPrice(LPVal, NMID, repriceFlg) {
    try {
      var LP = '';
      this.priceBtnActive =  'N' //Added by ApurvaK||04-Apr-2023|| Remove multiple clicks pn Price button
      if (LPVal == 'All') {
      ...
      const that = this;
      const parameters = [{
        productCode: this.templateName,//'YieldEnhancement',
          ...
          if (!that.clearFlag) {
            ...
            that.interval = setInterval(async () => {
                ...
                if (!quoteResponseCheck) {
                  quoteResponseCheck = await that.YEPriceResponse(that.PPDetails);
                }
                  ...
                  //   if (that.priceProvidersArr[i].price === '-' || that.priceProvidersArr[i].price === '') {
  ...
  async YEPriceResponse(PPDetails) {
    try {
      ...
      return await this.http.post(webMethod, parameters).toPromise().then((data: any) => {
          that.Prices = data;
          that.commonfunctions.setYEReceivedPrices(that.Prices, 1);
...
//END || Added by Varsha G || To clear multiple intervals || 23-Jun-2023
  ...
  reset() {
    try {
      ...
      this.priceProvidersArr.forEach(element => {
        ...
        element.MatchBtnDisable= false; //Added by Apurva K|| 22-Jan-2023
        element.MatchSentMsg = ''//Added by Apurva K|| 16-Jan-2023 || HSBCECCLI-32
      }
      );
  ...
  autocallfrombasedonFreqnTenor() {
    ...
    //Added by Apurva K|| 26-Dec-2023|| FIN1EURINT-573
    if(this.expshift == '1M'){
      this.cpnFreq = 'Monthly'
    }
...
//modified by PoojaW
  async Save() {
    try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
        ...
        var res : any;
       await this.apifunctions.BBVASaveQuotes('Single Pricer', strXml, this.portfolioName, '', 'YieldEnhancement', AppConfig.settings.oRes.userName).then((data:any)=>{res = data;});
        if (res) {
          if (res.errorMessage === '') {
            ...
            this.portfolioIdArr = [];
             await this.apifunctions.BBVAGetPortfolio('YieldEnhancement', 'Single Pricer').then((data:any)=>{
  ...
  async generateSaveXML() {
	...
	this.commonfunctions.generateFlexiXml({});
      ...
      this.tempXML += '<Record>' +
        ...
        '<SubTemplate>' + (this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'YieldEnhancement') + '</SubTemplate>' +
  ...
  async validationOnButton() {
    try {
      ...
      //   const resMsg = this.apifunctions.noncallValidation(this.autoNonCall, (this.expshift === 'Custom' ? this.customTenor : this.expshift),
      //     this.autoFreq.substr(0, this.autoFreq.length - 1));
      //   if (resMsg !== '') {
      //      this.ErrorMsgTop = resMsg;
      //     document.getElementById('txtautoNonCall').classList.add('error');
      //     $('<div class="error-input"></div><div class="validate-popup active"><span> ' + this.ErrorMsgTop + '</span></div>').insertAfter("#txtautoNonCall")
      //     $('.validate-popup').delay(5000).fadeOut('slow');
      //   }
    ...
    //End - Added by Apurva K|| 08-Aug-2023|| HSBCECCLI-21 | Validation on Price btn click
      ...
      //  this.checkNotionalRes = this.apifunctions.BBVACheckNotional(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'YieldEnhancement', this.ddlNoteCcy);
      ...
      // else {
      //   if (this.cpnType === 'No Coupon' && this.cpnType !== 'Fixed Coupon' && this.SolveForvalue !== 'CouponBarrier') {
      //      this.ErrorMsgTop = 'Coupon trigger should be between ' + this.minCoupon + '% and ' + this.maxCoupon + '%.';
      //     document.getElementById('txtCouponBarrier').classList.add('error');
      //     $('<div class="error-input"></div><div class="validate-popup active"><span> ' + this.ErrorMsgTop + '</span></div>').insertAfter("#txtCouponBarrier")
      //     $('.validate-popup').delay(5000).fadeOut('slow');
      ...
      // Added validation for Put Strike and KI Barrier || PriyaL || 09Apr2022 || Assigned by Pranav D.
      if (this.SolveForvalue !== 'PutStrike') {
        if (this.Strike === '') {
          this.ErrorMsgTop = 'Please enter valid put strike.';
          document.getElementById('txtStrike').classList.add('error');
          $('<div class="error-input"></div><div class="validate-popup active"><span> ' + this.ErrorMsgTop + '</span></div>').insertAfter("#txtStrike")
          $('.validate-popup').delay(5000).fadeOut('slow');
      ...
      // if (this.SolveForvalue !== 'KI') {
      // changed by Suvarna P || 27Apr2022 || KI barrier validation issue assigned by Pranav D.
      if (this.SolveForvalue !== 'KI' && this.barrierType !== 'None') {
        if (this.barrierLevel === '') {
          this.ErrorMsgTop = 'Please enter valid KI barrier.';
          document.getElementById('txtKI').classList.add('error');
          $('<div class="error-input"></div><div class="validate-popup active"><span> ' + this.ErrorMsgTop + '</span></div>').insertAfter("#txtKI")
          $('.validate-popup').delay(5000).fadeOut('slow');
  ...
  setPrevQuoteData1(cloneData, viewOnly) {
    try {
      ...
      // added by Suvarna P || 19Apr2022 || issue while clonning from prev quote || assigned by Pranav D || start
      this.priipsYN = cloneData.priipsYN[0];
      this.language = cloneData.language[0].replace(/~/g, ',')
      this.country = cloneData.country[0].replace(/~/g, ',')
      // added by Suvarna P || 19Apr2022 || issue while clonning from prev quote || assigned by Pranav D || end
      ...
      // const share1 = { Name: cloneData.FirstUnderlyingCode1[0] };
      ...
      this.stkshift = cloneData.StkShifter[0] === 'Today' ? '0B' : 'Fwd'; // Modified field mapping || Priya L || 30Mar2022 || assigned by Pranav D
      ...
      // added by Suvarna P || 20Apr2022 || prev quote clone issue for custom tenor || assigned by Pranav D || start
      // this.expshift = cloneData.ExpShifter[0].toUpperCase();   // converted to uppercase  || Priya L || 01Apr2022 || assigned by Pranav D
      ...
      // added by Suvarna P || 20Apr2022 || prev quote clone issue for custom tenor || assigned by Pranav D || end

      this.SettlementMethod = cloneData.SettlementMethod[0];
      this.publicOrPrivate = cloneData.publicOrPrivate[0];
      // this.CouponBarrierType = cloneData.CouponBarrierType[0];
      // this.changeAutoFreqOnTenor();
      ...
      // commented below fields  as not required in YE, and resolved cloning issue || Priya L || 02May2022
      ...
      // this.autoTrigger = cloneData.autoTrigger[0];//KOBarrier[0];
      ...
      });//Changed by Jyoti S || 27-Jul-2023
      ...
      //Parameters changed by Jyoti S || 06-Apr-2023
      ...
      if (viewOnly) {
        ...
        // this.onBehalfOf = cloneData.OnBehalfOf[0]; // Modified field mapping || Priya L || 01Apr2022 || assigned by Pranav D
        // this.GetClientProdDetailsArr = this.apifunctions.GetClientProdDetails(this.onBehalfOf);
        // if (this.GetClientProdDetailsArr !== undefined && this.GetClientProdDetailsArr.length > 0) {
        //   if (this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'YieldEnhancement') > -1) {
        //     this.mappedformatlist = (this.GetClientProdDetailsArr[this.GetClientProdDetailsArr.findIndex(record => record.CPM_Product === 'YieldEnhancement')].CPM_Format).toString().split(',');
        ...
        //   }
        // }
        // this.stkdate = this.datepipe.transform(cloneData.InputStrikeDate[0].split(' ')[0], 'yyyy-MM-dd');
        // this.settdate = this.datepipe.transform(cloneData.InputSettlementDate[0].split(' ')[0], 'yyyy-MM-dd');
        // this.expdate = this.datepipe.transform(cloneData.ComputedTenorValue[0].split('T')[0], 'yyyy-MM-dd');
        ...
        // this.rfqID = cloneData.RFQIDcsv[0]; //  //RFQIDcsv
        // this.rfqID = cloneData.RFQIDcsv[0].substring(0, cloneData.RFQIDcsv[0].toString().length - 2); //  //RFQIDcsv
        ...
        //     ? 'Rejected' : cloneData.SolveForValue[0], solveFor: cloneData.SolveForCode[0]
        // }];

        // switch (cloneData.SolveForCode[0]) {
        //   case 'Strike':
        ...
        //     break;
        //   case 'KO':
        //     this.autoTrigger = cloneData.SolveForValue[0]; // this.sortedAllPrices[0].Price;;
        //     this.replySolveFor = 'Autocall Trigger';
        //     break;
        //   case 'RebateCoupon':
        //     this.autocallCoupon = cloneData.SolveForValue[0]; // this.sortedAllPrices[0].Price;;
        ...
        //     break;
        //   case 'ERCoupon':
        //     this.ERCoupon = cloneData.SolveForValue[0]; // this.sortedAllPrices[0].Price;;
  ...
  async RequestTermsheet() {
    try {
      ...
      const res: any = await this.apifunctions.termsheetSender(this.sortedAllPrices[0].lp, this.sortedAllPrices[0].rfq, 'YieldEnhancement', this.language, this.country);
...
//Changed by Jyoti S || 31-May-2023 || END
  ...
  async RequestKIDonView() {
    try {
      ...
      this.sortedAllPrices[0].KIDLoadFlag = true;
        const res: any = await this.apifunctions.KIDSender(this.sortedAllPrices[0].lp, this.sortedAllPrices[0].rfq, 'YieldEnhancement', this.language, this.country);
      ...
      if (res?.status.toLowerCase() === 'success' || res?.status.toLowerCase() === 'awaited') {
        ...
        const KIDInterval = setInterval(async() => {
          if (TSTimeLeft > 0) {
            ...
            that.sortedAllPrices[0].KIDDisableFlag = false;
            that.reqSuccessMsg = 'KID response is taking longer than expected. Please try again later';
            clearInterval(KIDInterval);
          }
        }, 10000);
  ...
  showsaveSharePopup() {
    try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
        ...
        this.currentowner = AppConfig.settings.oRes.userName  //Added by Apurva K
        //this.currentowner = (this.commonfunctions.getLoggedInUserName())[0].UserId;
      }
  ...
  async scheduleSend() {
    try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
        ...
        // Removed onbehalf dependency || PriyaL || 25Apr2022 || Assigned by Pranav D
        // const res = this.apifunctions.SchedulePrice(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'YieldEnhancement', xmlstr,
        ...
        const res = await this.apifunctions.SchedulePrice(this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'YieldEnhancement', xmlstr,
    ...
    this.priceProvidersArr[index].interval1 = setInterval(() => {
      ...
      {  
        ...
        this.priceProvidersArr[index].loadFlag = false
         //Start - Added by Apurva K/Kaustubh S for HSBCECCLI-32 ||11-Aug-2023||23-Mar-2024
         ...
         //End - Added by Apurva K/Kaustubh S for HSBCECCLI-32 ||11-Aug-2023||23-Mar-2024
        clearInterval(this.priceProvidersArr[index].interval1);
      }
  ...
  async RequestKID(index: any) {
    try {
      ...
      // Removed onbehalf dependency || PriyaL || 25Apr2022 || Assigned by Pranav D
       // const errorMsg: any= await this.apifunctions.termsheetSender(this.priceProvidersArr[index].id, '', 'YieldEnhancement', 'YieldEnhancement', 'KID');

      // if (errorMsg === '') {
        const res: any = await this.apifunctions.KIDSender(this.priceProvidersArr[index].lp, this.priceProvidersArr[index].rfq, 'YieldEnhancement', this.language, this.country);
  ...
  async RequestTS(index: any) {
    try {
      ...
      // Removed onbehalf dependency || PriyaL || 25Apr2022 || Assigned by Pranav D
      // const errorMsg : any = await this.apifunctions.termsheetSender(this.priceProvidersArr[index].id, '', this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'YieldEnhancement',
      //   this.templateMappingArr.length > 0 ? this.templateMappingArr[0].subTemplate : 'YieldEnhancement', 'TS');
      // if (errorMsg === '') {
        const res: any = await this.apifunctions.termsheetSender(this.priceProvidersArr[index].lp, this.priceProvidersArr[index].rfq, 'YieldEnhancement', this.language, this.country);
      //Changes by Apurva K|| 18-May-2023 || for Awaited response in request TS
        if ((res?.status.toLowerCase() === 'success' || res?.status.toLowerCase() === 'awaited') && (this.priceProvidersArr[index].price !== "-")) {
  ...
  async showOrderDetailsFromPrevQuote() {
    try {
      ...
      // this.txtIBPrice = this.dataItemAccordian.ER_SolveFor === 'Price(%)' ? this.dataItemAccordian.EP_OfferPrice :this.dataItemAccordian.EP_Price  ;
  ...
  */
  ...
  //Added the missing changes from Autocallable|| Apuvra K|| 01-Apr-2024||FIN1EURINT-710
  async confirmOrder() {
    ...
    for (let i = 0; i < this.defaultvaluesArr.length; i++) {
      switch (this.defaultvaluesArr[i].Control_Name) {
        ...
        case 'Notional':
          ...
          this.NotionalUpdatedBy = this.defaultvaluesArr[i].Updated_By;
          this.NotionalUpdatedAt = this.defaultvaluesArr[i].Updated_At;
          // ////console.log(this.Notional,"defaultNotionalval");
          this.notionalYN = this.defaultvaluesArr[i].ActiveYN;
        ...
        case 'ReofferPrice':
          ...
          this.ReofferPriceUpdatedBy = this.defaultvaluesArr[i].Updated_By;
          this.ReofferPriceUpdatedAt = this.defaultvaluesArr[i].Updated_At;
         
          this.ReofferPriceUpdatedAt = (new Date(Date.parse(this.ReofferPriceUpdatedAt + ' UTC'))).toLocaleString();
          // ////console.log(this.Notional,"defaultNotionalval");
          this.reofferPriceYN = this.defaultvaluesArr[i].ActiveYN;
          break;
          ...
          case 'Upfront':
            if(this.defaultvaluesArr[i].Default_Value == 'undefined' || undefined){
              ...
              this.UpfrontIBPrice = this.defaultvaluesArr[i].Default_Value;
            }
            this.UpfrontUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.UpfrontUpdatedAt = this.defaultvaluesArr[i].Updated_At;
           
            this.UpfrontUpdatedAt = (new Date(Date.parse(this.UpfrontUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.upfrontYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'IssuePrice':
            ...
            this.IssuePriceUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.IssuePriceUpdatedAt = this.defaultvaluesArr[i].Updated_At;
            // ////console.log(this.Notional,"defaultNotionalval");
  
            this.IssuePriceUpdatedAt = (new Date(Date.parse(this.IssuePriceUpdatedAt + ' UTC'))).toLocaleString();
            this.issuePriceYN = this.defaultvaluesArr[i].ActiveYN;
            ////console.log("ISSUEPRICE1",this.issuePriceYN,this.issuePrice)
            break;
  
          case 'BasketType':
            this.basketType = this.defaultvaluesArr[i].Default_Value;
            this.basketTypeYN = this.defaultvaluesArr[i].ActiveYN;
            this.basketTypeDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
  
            this.BasketTypeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.BasketTypeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
          ...
          case 'PublicPrivate':
            ...
            this.publicPrivateYN = this.defaultvaluesArr[i].ActiveYN;
            this.publicPrivateDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
            this.publicOrPrivate = this.defaultvaluesArr[i].Default_Value;
            this.PublicPrivateUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.PublicPrivateUpdatedAt = this.defaultvaluesArr[i].Updated_At;
          ...
          case 'Quanto':
            ...
            this.QuantoUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.QuantoUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.QuantoUpdatedAt = (new Date(Date.parse(this.QuantoUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.quantoYN = this.defaultvaluesArr[i].ActiveYN;
            break;
  
          case 'Listed':
            this.listed = this.defaultvaluesArr[i].Default_Value;
            // ////console.log(this.Notional,"defaultNotionalval");
            this.listedYN = this.defaultvaluesArr[i].ActiveYN;
            this.ListedUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.ListedUpdatedAt = this.defaultvaluesArr[i].Updated_At;
          ...
          case 'StockExchange':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.stockExchangeYN = this.defaultvaluesArr[i].ActiveYN;
            this.stockExchangeDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
            this.StockExchangeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.StockExchangeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
          ...
          case 'Priips':
            this.priipsPopupYN = this.defaultvaluesArr[i].Default_Value;
            // ////console.log(this.Notional,"defaultNotionalval");
            this.priipsPopupYN = this.defaultvaluesArr[i].ActiveYN;
            this.PriipsUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.PriipsUpdatedAt = this.defaultvaluesArr[i].Updated_At;
          ...
          case 'Country':
            ...
            ////console.log(this.countryDropdown, "languageDropdown");
          ...
          case 'Termsheet':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.termsheetYN = this.defaultvaluesArr[i].ActiveYN;
            this.termsheetDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
            this.TermsheetUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.TermsheetUpdatedAt = this.defaultvaluesArr[i].Updated_At;
        ...
        case 'StrikeShifterDate':
          ...
          this.ShifterDateYN = this.defaultvaluesArr[i].ActiveYN;
          this.StrikeShifterDateUpdatedBy = this.defaultvaluesArr[i].Updated_By;
          this.StrikeShifterDateUpdatedAt = this.defaultvaluesArr[i].Updated_At;
        ...
        case 'IssueDateOffset':
          ...
          this.issueDateOffsetYN = this.defaultvaluesArr[i].ActiveYN;
          ////console.log( this.issueDateOffsetYN, "issueDateDropdownYN");
          this.issueDateDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
          this.IssueDateOffsetUpdatedBy = this.defaultvaluesArr[i].Updated_By;
          this.IssueDateOffsetUpdatedAt = this.defaultvaluesArr[i].Updated_At;

          this.IssueDateOffsetUpdatedAt = 
          (new Date(Date.parse(this.IssueDateOffsetUpdatedAt + ' UTC'))).toLocaleString();
          break;
          ...
          case 'FundType':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.FundTypeYN = this.defaultvaluesArr[i].ActiveYN;
            this.FundTypeDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
            this.FundTypeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.FundTypeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.FundTypeUpdatedAt = 
            (new Date(Date.parse(this.FundTypeUpdatedAt + ' UTC'))).toLocaleString();
          ...
          case 'FundingFrequency':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.FundFreqYN = this.defaultvaluesArr[i].ActiveYN;
            this.fundFreqDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
            this.FundingFrequencyUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.FundingFrequencyUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            
            this.FundingFrequencyUpdatedAt = 
            (new Date(Date.parse(this.FundingFrequencyUpdatedAt + ' UTC'))).toLocaleString();
          ...
          case 'FundRateSpread':
            ...
            this.FundingRateSpreadUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.FundingRateSpreadUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.FundingRateSpreadUpdatedAt = 
            (new Date(Date.parse(this.FundingRateSpreadUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.RateSpreadYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'PutableFrequency':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.putableFreqYN = this.defaultvaluesArr[i].ActiveYN;
            this.putableFreqDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
            
            this.PutableFrequencyUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.PutableFrequencyUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.PutableFrequencyUpdatedAt = 
            (new Date(Date.parse(this.PutableFrequencyUpdatedAt + ' UTC'))).toLocaleString();
          ...
          case 'CallableFrequency':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.callableFreqYN = this.defaultvaluesArr[i].ActiveYN;
            this.callableFreqDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
            this.CallableFrequencyUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.CallableFrequencyUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.CallableFrequencyUpdatedAt = 
            (new Date(Date.parse(this.CallableFrequencyUpdatedAt + ' UTC'))).toLocaleString();
          ...
          case 'AutocallBarrier':
            ...
            this.AutocallBarrierUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.AutocallBarrierUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.AutocallBarrierUpdatedAt = 
            (new Date(Date.parse(this.AutocallBarrierUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.autocallBarrierYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'StepUpDown':
            ...
            this.StepUpDownUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.StepUpDownUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.StepUpDownUpdatedAt = 
            (new Date(Date.parse(this.StepUpDownUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.stepdownYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'PutableBarrierLevel':
            ...
            this.PutableBarrierLevelUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.PutableBarrierLevelUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.PutableBarrierLevelUpdatedAt = 
            (new Date(Date.parse(this.PutableBarrierLevelUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.putableBarrierLevelYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'PutStrike':
            ...
            this.PutStrikeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.PutStrikeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.PutStrikeUpdatedAt = 
            (new Date(Date.parse(this.PutStrikeUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.putStrikeYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'ProtectionLvl':
            ...
            this.ProtectionLevelUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.ProtectionLevelUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.ProtectionLevelUpdatedAt = 
            (new Date(Date.parse(this.ProtectionLevelUpdatedAt + ' UTC'))).toLocaleString();
           
            this.protectionLvlYN = this.defaultvaluesArr[i].ActiveYN;
          ...
          case 'UpperPutStrike':
            ...
            this.UpperPutStrikeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.UpperPutStrikeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.UpperPutStrikeUpdatedAt = 
            (new Date(Date.parse(this.UpperPutStrikeUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.upperPutStrikeYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'PutGearring':
            ...
            this.PutGearringUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.PutGearringUpdatedAt = this.defaultvaluesArr[i].Updated_At;
            
            this.PutGearringUpdatedAt = 
            (new Date(Date.parse(this.PutGearringUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.putGearingYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'LowerPutStrike':
            ...
            this.LowerPutStrikeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.LowerPutStrikeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.LowerPutStrikeUpdatedAt = 
            (new Date(Date.parse(this.LowerPutStrikeUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.lowerPutStrikeYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'KIBarrier':
            ...
            this.BarrierLevelUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.BarrierLevelUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.BarrierLevelUpdatedAt = 
            (new Date(Date.parse(this.BarrierLevelUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.popupKIBarrierYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'PutSpreadGearing':
            ...
            this.PutSpreadGearingUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.PutSpreadGearingUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.PutSpreadGearingUpdatedAt = 
            (new Date(Date.parse(this.PutSpreadGearingUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.putSpreadGearingYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'CallStrike':
            ...
            this.CallStrikeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.CallStrikeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.CallStrikeUpdatedAt = 
            (new Date(Date.parse(this.CallStrikeUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.callStrikePercYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'LowerCallStrike':
            ...
            this.LowerCallStrikeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.LowerCallStrikeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.LowerCallStrikeUpdatedAt = 
            (new Date(Date.parse(this.LowerCallStrikeUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.lowerCallStrikePercYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'CallGearing':
            ...
            this.CallGearringUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.CallGearringUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.CallGearringUpdatedAt = 
            (new Date(Date.parse(this.CallGearringUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.callGearingYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'UpperCallStrike':
            ...
            this.UpperCallStrikeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.UpperCallStrikeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.UpperCallStrikeUpdatedAt = 
            (new Date(Date.parse(this.UpperCallStrikeUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.upperCallStrikeYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'CallSpreadGearing':
            ...
            this.CallSpreadGearingUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.CallSpreadGearingUpdatedAt = this.defaultvaluesArr[i].Updated_At;
            
            this.CallSpreadGearingUpdatedAt = 
            (new Date(Date.parse(this.CallSpreadGearingUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.callSpreadGearingYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'ERCouponPa':
            ...
            this.ERCouponPaUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.ERCouponPaUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.ERCouponPaUpdatedAt = 
            (new Date(Date.parse(this.ERCouponPaUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.ERCouponPaYN = this.defaultvaluesArr[i].ActiveYN;
            break;
          ...
          case 'CpnBarrierType':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.cpnBarrierTypeYN = this.defaultvaluesArr[i].ActiveYN;
            this.cpnBarrierTypeDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
            this.CouponBarrierTypeUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.CouponBarrierTypeUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.CouponBarrierTypeUpdatedAt = 
            (new Date(Date.parse(this.CouponBarrierTypeUpdatedAt + ' UTC'))).toLocaleString();
          ...
          case 'RangeAccuralFreq':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.rangeAccFreqYN = this.defaultvaluesArr[i].ActiveYN;
            this.rangeAccFreqDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
  
            this.RangeAccuralFreqUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.RangeAccuralFreqUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.RangeAccuralFreqUpdatedAt = 
            (new Date(Date.parse(this.RangeAccuralFreqUpdatedAt + ' UTC'))).toLocaleString();
          ...
          case 'PeriodicCpnFreq':
            ...
            // ////console.log(this.Notional,"defaultNotionalval");
            this.periodicCpnFreqYN = this.defaultvaluesArr[i].ActiveYN;
            this.cpnFreqDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
  
            this.PeriodicCpnFreqUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.PeriodicCpnFreqUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.PeriodicCpnFreqUpdatedAt = 
            (new Date(Date.parse(this.PeriodicCpnFreqUpdatedAt + ' UTC'))).toLocaleString();
            ////console.log( "cpnFreqDropdown",this.cpnFreqDropdown, this.cpnFreq);
            break;
  
            case 'FloatingRefIndex':
            this.cpnFltRef = this.defaultvaluesArr[i].Default_Value;
            // ////console.log(this.Notional,"defaultNotionalval");
            this.floatingRefIndexYN = this.defaultvaluesArr[i].ActiveYN;
            this.floatingFreIndexDropdown = this.defaultvaluesArr[i].Selectedvaluecsv.split(',');
  
            this.FloatingRefIndexUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.FloatingRefIndexUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.FloatingRefIndexUpdatedAt = 
            (new Date(Date.parse(this.FloatingRefIndexUpdatedAt + ' UTC'))).toLocaleString();
            ////console.log( "cpnFreqDropdown",this.cpnFreqDropdown, this.cpnFreq);
            break;
          ...
          case 'CpnBarrierLvl':
            ...
            this.CouponBarrierLevelUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.CouponBarrierLevelUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.CouponBarrierLevelUpdatedAt = 
            (new Date(Date.parse(this.CouponBarrierLevelUpdatedAt + ' UTC'))).toLocaleString();
            // ////console.log(this.Notional,"defaultNotionalval");
            this.cpnBarrierLvlYN = this.defaultvaluesArr[i].ActiveYN;
            break;
            ...
            case 'LowerCouponBarrier':
              if(this.defaultvaluesArr[i].Default_Value == 'undefined' || undefined){
                ...
                this.lowCpnBarrier = this.defaultvaluesArr[i].Default_Value;
              }
             
              this.LowerCouponBarrierUpdatedBy = this.defaultvaluesArr[i].Updated_By;
              this.LowerCouponBarrierUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
              this.LowerCouponBarrierUpdatedAt = 
              (new Date(Date.parse(this.LowerCouponBarrierUpdatedAt + ' UTC'))).toLocaleString();
              // ////console.log(this.Notional,"defaultNotionalval");
              this.LowerCouponBarrierYN = this.defaultvaluesArr[i].ActiveYN;
              break;
              ...
              case 'UpperCouponBarrier':
                if(this.defaultvaluesArr[i].Default_Value == 'undefined' || undefined){
                  ...
                  this.upperCpnBarrier = this.defaultvaluesArr[i].Default_Value;
                }
             
                this.UpperCouponBarrierUpdatedBy = this.defaultvaluesArr[i].Updated_By;
                this.UpperCouponBarrierUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
                this.UpperCouponBarrierUpdatedAt = 
              (new Date(Date.parse(this.UpperCouponBarrierUpdatedAt + ' UTC'))).toLocaleString();
              // ////console.log(this.Notional,"defaultNotionalval");
              this.UpperCouponBarrierYN = this.defaultvaluesArr[i].ActiveYN;
          ...
          case 'CouponPaPerc':
            ...
            this.CouponPaPercUpdatedBy = this.defaultvaluesArr[i].Updated_By;
            this.CouponPaPercUpdatedAt = this.defaultvaluesArr[i].Updated_At;
  
            this.CouponPaPercUpdatedAt = 
            (new Date(Date.parse(this.CouponPaPercUpdatedAt + ' UTC'))).toLocaleString();
  
            // ////console.log(this.Notional,"defaultNotionalval");
            this.couponPaPercYN = this.defaultvaluesArr[i].ActiveYN;
            ...
            case 'CouponInFine':
              ...
              this.CouponInFineUpdatedBy = this.defaultvaluesArr[i].Updated_By;
              this.CouponInFineUpdatedAt = this.defaultvaluesArr[i].Updated_At;
    
              this.CouponInFineUpdatedAt = 
              (new Date(Date.parse(this.CouponInFineUpdatedAt + ' UTC'))).toLocaleString();
              // ////console.log(this.Notional,"defaultNotionalval");
              this.cpnInFineYN = this.defaultvaluesArr[i].ActiveYN;
              break;
              ...
              //   }
              //   else{
              //     this.cpnMultiplier = this.defaultvaluesArr[i].Default_Value;
              //   }
             
              //   this.MultiplierUpdatedBy = this.defaultvaluesArr[i].Updated_By;
              //   this.MultiplierUpdatedAt = this.defaultvaluesArr[i].Updated_At;
    
              //   this.MultiplierUpdatedAt = 
              //   (new Date(Date.parse(this.MultiplierUpdatedAt + ' UTC'))).toLocaleString();
              // // ////console.log(this.Notional,"defaultNotionalval");
              // this.MultiplierYN = this.defaultvaluesArr[i].ActiveYN;
              ...
              //   }
              //   else{
              //     this.cpnFloor = this.defaultvaluesArr[i].Default_Value;
              //   }
             
              //   this.FloorUpdatedBy = this.defaultvaluesArr[i].Updated_By;
              //   this.FloorUpdatedAt = this.defaultvaluesArr[i].Updated_At;
    
                
              //   this.FloorUpdatedAt = 
              //   (new Date(Date.parse(this.FloorUpdatedAt + ' UTC'))).toLocaleString();
              //   // ////console.log(this.Notional,"defaultNotionalval");
              //   this.FloorYN = this.defaultvaluesArr[i].ActiveYN;
              ...
              //   break;
                ...
                //   }
                //   else{
                //     this.cpnCap = this.defaultvaluesArr[i].Default_Value;
                //   }
                  
                //   this.CapUpdatedBy = this.defaultvaluesArr[i].Updated_By;
                //   this.CapUpdatedAt = this.defaultvaluesArr[i].Updated_At;
    
                //   this.CapUpdatedAt = 
                //   (new Date(Date.parse(this.CapUpdatedAt + ' UTC'))).toLocaleString();
                //   // ////console.log(this.Notional,"defaultNotionalval");
                //   this.CapYN = this.defaultvaluesArr[i].ActiveYN;
  ...
  portfolioChange() {
    ...
    if (this.portfolio !== '') {
      ...
      setTimeout(async () => {
        ...
        const saveQuoteData:any = await this.apifunctions.getRedirectionData(this.portfolio,'YieldEnhancement');
        if (saveQuoteData.length === 0) {
          this.ErrorMsgTop = 'No data found for this record.';
        } else {
  ...
  async DeleteTemplate() {
    try {
      ...
      this.successMsg = ''
      if (this.portfolio !== '') {
        const res = await this.apifunctions.BBVADeletePortfolio(this.portfolio, 'YieldEnhancement');
        if (res) {
          this.saveFlag = true;
  ...
  SelectedTab(Tab: any) {
    ...
    // this.commonfunctions.setAccord(6, 'YieldEnhancement')
  }
  selectRecord = {};
  ...
  lauchProduct(priceRecord) {
    ...
    this.selectRecord['Template_Code'] = 'YieldEnhancement';
  ...
  async updatePortfolio() {
    try {
      ...
      if (this.ErrorMsg === '' && this.ErrorMsgTop === '') {
        ...
        this.reqSuccessMsg = '';
        const strXml = '<Details>' +await this.generateSaveXML() + '</Details>';
        const res :any = await this.apifunctions.BBVASaveQuotes('Single Pricer', strXml, this.portfolioName, this.portfolio, 'YieldEnhancement', AppConfig.settings.oRes.userName);
        if (res) {
          if (res.errorMessage === '') {
  ...
  // function to disable fields based on default values - added by Priya L. on 07Feb2022 - assigned by Pranav D.
            ...
            if (!that.clearFlag) {
              ...
              let quoteResponseCheck = false;
                that.interval = setInterval(async () => {
                  ...
                  if (!quoteResponseCheck) {
                    quoteResponseCheck = await that.YEPriceResponse(that.PPDetails);
                   // if(this.sortedAllPrices[0].MatchBtnDisable)
                   // this.sortedAllPrices[0].MatchBtnDisable = true;
                  }
