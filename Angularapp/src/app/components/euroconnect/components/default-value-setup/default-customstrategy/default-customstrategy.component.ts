...
@Component({
  selector: 'app-default-customstrategy',
  templateUrl: './default-customstrategy.component.html',
  styleUrls: ['./default-customstrategy.component.scss']
})
export class DefaultCustomstrategyComponent implements OnInit {
 ...
 // { 'Key': 'Italy', 'Value': 'Italy' },
  ...
  solveForArr: any =
    [
      ...
      { 'Key': 'Reoffer (%)', 'Value': 'Reoffer' } //Added by Apurva K
    ];
  ...
  constructor(public elem: ElementRef, public commonfunctions: EcCommonService,
    ...
    private renderer: Renderer2) {
      try {
        ...
        this.applicationID = "CustomPayoffs";
        this.flag = false;
        this.shares = [];
        // this.ddlNoteCcy = 'EUR';
   ...
   ngOnInit() {
    ...
    try {
      this.pageloadflag=true; //changed by AdilP FIN1EURINT-506 || 05-07-2023 
      $('#loading').show();
      setTimeout(async () => {
        ...
        this.shares = await this.apifunctions.BBVALoadShares('EQ,IDX', "", "CustomPayoffs");
          // }
        ...
        this.PopupCcyArr = [];
        if (this.CCY && this.CCY.length > 0) {
          this.CCY.forEach((ele,idx)=>{
            this.PopupCcyArr.push({ 'Key': this.CCY[idx], 'Value': this.CCY[idx]}); 
          })
          // for (let i = 0; i < this.CCY.length; i++) {
          //   this.PopupCcyArr.push({ 'Key': this.CCY[i], 'Value': this.CCY[i]});      
          // }
          }
        // this.ddlNoteCcy = 'EUR';
       ...
       // this.fnGetValidation();
        ...
        this.allDefaultValues = await this.apifunctions.GetEntityDefaultValues("", AppConfig.settings.oRes.userID, 'CustomPayoffs');
        ...
        if (this.allBooksData && this.allBooksData.length > 0) {
          ...
          console.log(this.onBehalfOf, "OnBehalfOf");
      ...
      switch (this.templateMappingArray[i]) {
        ...
        case 'IssueDateOffset':
          if (this.issueDateOffsetYN == 'Y'|| this.issueDateOffsetYN == '') {
            ...
            + (this.paymentshift === 'Custom' ? (this.customSetDate + 'B') : (this.paymentshift === 'T + 10' ? '10' : '5')) +
            '</IssueDateOffset>';
          }
          break;
