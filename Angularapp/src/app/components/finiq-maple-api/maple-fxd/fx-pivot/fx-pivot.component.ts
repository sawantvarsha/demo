...
@Component({
  selector: 'app-fx-pivot',
  templateUrl: './fx-pivot.component.html',
  styleUrls: ['./fx-pivot.component.scss']
})
export class FxPivotComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.mapleapi.maplegetccy("4","10","PIVOT","FXOSEN","","Y","","","","TOK");
    ...
    //     }
    //     console.log("Maple FXD currList(Pivot) = ",this.currList);
    //   }
    // })
  
    // this.mapleapi.maplegetproddetails("PIVOT");
    // this.mapleapi.prodDetails.subscribe(res => {
    //   if(res)
    //   {           
    //     console.log("Maple FXD product details(Pivot) = ",res);
    //   }
    // })

    // this.mapleapi.maplegetdates("EUR","USD","EUR - USD","12-Oct-2021","00","4","6M","Monthly","Monthly","33","FXAQ","FXOSEN","TOK","32");
    // this.mapleapi.datesFXD.subscribe(res => {
    //   if(res)
    //   {           
    //     console.log("Maple FXD DATES = ",res);
    //   }
    // })
    ...
    this.mapleapi.maplegetbidask(this.selectedCcy,"PIVOT","FXOSEN",this.tileID);
    this.mapleapi.bidAskFXD.subscribe(res => {
     if(res!= "" && this.tileID == "102")
      {                   
        let bidAskres = res[0].Get_FinIQ_BidAsk_WrapperResult;

        this.bidPivot = bidAskres.BidRate;
        this.bidPivot = Number(this.bidPivot).toFixed(4);
        this.askPivot = bidAskres.AskRate;
        console.log("Maple FXD bid ask(FXoptions) = ",this.bidPivot,this.askPivot);
