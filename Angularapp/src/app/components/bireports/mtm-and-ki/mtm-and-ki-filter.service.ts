...
export class MtmAndKiFilterService {
  ...
  min_KI: number = 0
  max_KI: number = 0
  current_min_KI: number = this.min_KI
  current_max_KI: number = this.max_KI
  min_MTM: number = 0
  max_MTM: number = 0
  ...
  setData(apiData: any[]) {
    //this.data =apiData
     this.data = apiData.filter((data) => data["WorstOfPerformance"] !== null && isEmptyObject(data['Trace_KI_Distance']));
    this.setProperties()
  ...
  getData(): dataType[] {
    ...
    this.filteredData.forEach((data) => {
      ...
      tempData = {
        ...
        "MTM_Premium": data[this.mtmPremium],
        "ISIN": data[this.ISIN],
  ...
  setProperties() {
    ...
    this.max_MTM = mtm_list[KI_list.length - 1]
  ...
  filterData() {
    this.clientFilters();
    this.filteredData = this.clientsfilteredData.filter((data) => (data[this.traceKIDistance] >= this.current_min_KI && data[this.traceKIDistance] <= this.current_max_KI) && (data[this.mtmPremium] >= this.current_min_MTM && data[this.mtmPremium] <= this.current_max_MTM)
    )
    let tempData: any
  ...
  clientFilters() {
    try {
      ...
      //       this.clientsfilteredData = this.data.filter((data) => {
      //         if(JSON.stringify(data['CustomerName']) === '{}'){
      //           return false;
      //         }else{
      //           return data['CustomerName'].includes(this.selectedClient)
      //         }
      //       })
