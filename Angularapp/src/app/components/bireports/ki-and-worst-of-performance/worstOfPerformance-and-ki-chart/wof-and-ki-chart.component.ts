...
@Component({
  selector: 'app-wof-and-ki-chart',
  templateUrl: './wof-and-ki-chart.component.html',
  styleUrls: ['./wof-and-ki-chart.component.css']
})
export class KiAndWorstOfPerformanceChartComponent implements AfterViewInit {
  constructor(private service: KiAndWorstOfPerformanceService, private dashboardService: DashboardServiceService, private bIReportsComponent: BIReportsComponent, private kiAndWorstOfPerformanceComponent: KiAndWorstOfPerformanceComponent, private datepipe: DatePipe, private chartService: ChartService, private themeService: ThemeserviceService) {
  }
...
display(){
  ...
  ngOnInit(): void {
    ...
    //Added by Adil || 08-09-2023 || START
    ...
    if (this.chart) {
      ...
      this.chart.config.data.datasets[1].backgroundColor = Array(this.woflist.length).fill(this.service.hex2)
  ...
  setComponentProperties() {
    ...
    Only20Data.forEach((data) => {
      ...
      temp_wof_list.push(data["WorstOfPerformance"]);
      temp_isinlist.push(data["ISIN"]);
      tempProductList.push(data["Product_Name"]);
      tempWorstList.push(data["WorstOfUnderlying"]);
    })

    let maxWof = Math.max(...temp_wof_list)
    let maxKi = Math.max(...temp_ki_list)
    let minWof = Math.min(...temp_wof_list)
    let minKi = Math.min(...temp_ki_list)
    this.maxElement = Math.max(maxWof, maxKi)
    this.minElememt = Math.min(minWof, minKi)

    this.ki_list =temp_ki_list
    this.woflist =temp_wof_list
  ...
  createChart() {
    try {
      let chartStatus = Chart.getChart("ki-chart"); // <canvas> id
      if (chartStatus != undefined) {
        this.chart= Chart.getChart("ki-chart");
        this.chart.destroy();
      }
      this.chart = new Chart("ki-chart", {
        type: 'bar',
        data: {
        ...
        options: {
          ...
          onClick: (event, elements) => {
            ...
            if (this.prevElement !== this.selected || this.select === true) {
              ...
              var backgroundcolor2 = Array(this.woflist.length).fill(this.ColorLuminance(this.hex2, -0.5))
              ...
              this.chart.config.data.datasets[1].backgroundColor = Array(this.woflist.length).fill(this.hex2)
              this.select = true
              this.chart?.update()
            }
          }
          ...
          scales: {
            y: {
              ...
              title: {
                ...
                display: true,
                text: 'Protection and Worst Of Performance',
                font: {
                  size: 20
                }
              },
              beginAtZero: false,
              // min: Math.round(this.minElememt - 20),
  ...
  setChartProperties() {
    if (this.chart) {
      ...
      this.chart.config.data.datasets[1].data = this.woflist;
      // this.chart.config.data.datasets[0].backgroundColor = Array(this.ki_list.length).fill('rgba(57,114,158,255)')
      // this.chart.config.data.datasets[1].backgroundColor = Array(this.woflist.length).fill('rgba(37,82,114,255)')
      // this.chart.config.data.datasets[0].backgroundColor = Array(this.ki_list.length).fill(this.hex1)
      // this.chart.config.data.datasets[1].backgroundColor = Array(this.woflist.length).fill(this.hex2)
      // this.hex1 = '#39FF14'
      // this.hex2 = '#FF0099'
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  pinChart1(title) {
    this.kiAndWorstOfPerformanceComponent.pinned()
    ...
    const chart = {
      data, data1, productlist, worstList, labels, backgroundColor, backgroundColor1, borderColor, borderWidth, title, id, current_min_KI, current_max_KI, worstOfUnderlying, client, selectedOption, selectedOption1, bg1, bg2, date, inputString
    }
...
onColorChange(value: any) {
  ...
  this.chart.config.data.datasets[1].backgroundColor = Array(this.woflist.length).fill(this.invertHexColor(value))
