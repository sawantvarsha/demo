...
@Component({
  selector: 'app-sp-payments-recon',
  templateUrl: './sp-payments-recon.component.html',
  styleUrls: ['./sp-payments-recon.component.scss']
})
export class SpPaymentsReconComponent implements OnInit {
  ...
  StartDate:any;
  FixingStartDate: any; //Added by Apurva K||05-Feb-2024
  FixingDateDisplay: any; //Added by Apurva K||05-Feb-2024
  StartDateDisplay:any;
  EndDateDisplay:any;
  PaymentStartDate:any;//Added by Apurva K||05-Feb-2024
  PaymentDateDisplay:any;//Added by Apurva K||05-Feb-2024
  EndDate:any;
  display = 1;
  formatData:any;
  currencyData:any;
  ...
  constructor(
    ...
    this.selectedGroup='';
      ...
      this.StartDate = "";
      this.FixingStartDate =""; //Added by Apurva K||05-Feb-2024
      this.FixingDateDisplay = "";//Added by Apurva K||05-Feb-2024
      this.PaymentStartDate = "";//Added by Apurva K||06-Feb-2024
      this.PaymentDateDisplay = "";//Added by Apurva K||06-Feb-2024
  ...
  {
    let param:any={
      TemplateCode:"USP_Get_SP_Payments_Recon",
      EntityID:"27",
      UserId: AppConfig.settings.oRes?.userID,
      CustomerID:"",
      EventType:"",
    ...
    console.log("MAINNN DATA11",mainData1);
    if(mainData1){
      this.loader=false;
      let blotterData = [];
      ...
      this.formatData=blotterData.map(item=>item["Payment_Type"]).sort((a,b) => a > b ? 1 : -1)
      ...
      // });
    console.log("MAINNN Client",blotterData);
    ...
    console.log("MAINNN Subtutela",blotterData);
    ...
    console.log("MAINNN Zone",blotterData);
     ...
     // this.onKeydownRowsPerPage(10);
      ...
      this.totalCount = blotterData.length;
      //console.log('this.totalCount newGetPortfolioData : ' + this.totalCount);
    ...
    console.log("MAINNN Zone",blotterData);
      ...
      console.log(this.blotterData,"hello123")
      this.loader = false;
    }
    else{
      this.loader=false;
  ...
  getPortfolioData(){
    this.portfolioService.GetProductDetailsTimeline().subscribe(async (data)=>{
      ...
      if(data){
        ...
        //console.log('this.totalCount getPortfolioData : ' + this.totalCount);
        let mainData = blotterData;
        this.unFilteredBlotterData = mainData;
        this.filteredData = this.unFilteredBlotterData;
        //this.onKeydownRowsPerPage(10);
  ...
  {
    //console.log(this.FixingDateDisplay,"this.FixingDateDisplay")
   console.log("STARTT DATE",event);
    ...
    if (this.selectedExternal!= "") { 
      ...
      backupData = backupData.filter((s: any) => externalToSearch.some(exteranl_ref => (s.External.includes(exteranl_ref) || s.External.toLowerCase().includes(exteranl_ref.toLowerCase()))))
      console.log("selectedExternal ",backupData)
    }
    ...
    if (this.selectedFormat && this.selectedFormat != "" ) {
      ...
      // }
      //console.log(this.selectedFormat,"Format APURVA")
      ...
      if(formatArr?.length==this.formatData.length ){
        ...
        backupData = backupData.filter((s: any) => formatArr.some(Payment_Type => (s.Payment_Type.toString().toLowerCase().includes(Payment_Type) || s.Payment_Type.toLowerCase().includes(Payment_Type.toLowerCase()))))
      }
    }
    ...
    // }
     ...
     //Added by Apurva K||05-Feb-2024
    if (this.FixingDateDisplay != "" && this.FixingDateDisplay !== undefined) {
      backupData = backupData.filter((s) => new Date(s.Fixing_Date).getTime() === new Date(this.FixingDateDisplay).getTime());
      console.log("inside fixing display", backupData)
    }

     //Added by Apurva K||06-Feb-2024
     if (this.PaymentDateDisplay != "" && this.PaymentDateDisplay !== undefined) {
      backupData = backupData.filter((s) => new Date(s.Payment_Date).getTime() === new Date(this.PaymentDateDisplay).getTime());
      //backupData = backupData.filter((s: any) =>(new Date(s.Payment_Date)) == (new Date(this.PaymentDateDisplay)))
    }

    console.log("after fixing display", backupData)
   ...
   // this.portfolioSort(true);
    ...
    if(this.excelDataFlag){
      ...
      this.totalCount = backupData.length; //changed by Apurva to remove duplicate check as externals ae not unique
     // console.log('this.totalCount FilterForAll : ' + this.totalCount);
      // this.onKeydownRowsPerPage(this.pageSize);
  ...
  selectFixingDate(date) {
    try {
      console.log(date)
      if (date !== '') {
        this.FixingDateDisplay = this.datepipe.transform(date, 'dd-MMM-yyyy');
        //console.log(this.FixingDateDisplay,"this.FixingDateDisplay");
      } else {
        this.FixingStartDate = "";
        this.FixingDateDisplay = undefined;
      }
    } catch (error) { }
  }
  selectPaymentDate(date) {
    try {
      if (date !== '') {
        this.PaymentDateDisplay = this.datepipe.transform(date, 'dd-MMM-yyyy');
      } else {
        this.PaymentDateDisplay = undefined;
  ...
  clearClient() {
    ...
    //Added by Apurva K||05-Feb-2024
    if (this.FixingDateDisplay != "" && this.FixingDateDisplay !== undefined) {
      backupData = backupData.filter((s: any) =>(new Date(s.Fixing_Date)) >= (new Date(this.FixingDateDisplay)))
    }
    ...
    console.log(backupData,"backpdata")
    backupData=this.sortTable(backupData, "Start_Date");
    this.filteredBackData=backupData;
    this.portfolioSort(true);
    if(this.excelDataFlag){
      ...
      //console.log('this.totalCount clearClient : ' + this.totalCount);
      if(this.previousFilterfield!=undefined && this.previousFilterfield!="")
      {
        this.sort(this.previousFilterfield,false)
      }
  ...
  clearProduct() {
    ...
    this.formatData=backupData.map(item=>item["Payment_Type"]).sort((a,b) => a > b ? 1 : -1)
    this.formatData=[...new Set(this.formatData)];
    ...
    if(this.excelDataFlag){
      ...
      this.setSlidersValue();
      this.totalCount= backupData.length;
      //console.log('this.totalCount clearProduct : ' + this.totalCount);
      if(this.previousFilterfield!=undefined && this.previousFilterfield!="")
      {
        this.sort(this.previousFilterfield,false)
      }
  ...
  clearDateFilters() {
    ...
    if (this.formatData && this.selectedFormat != "" ) {
    ...
    if(this.excelDataFlag){
      ...
      //this.setSlidersValue();
      this.totalCount= backupData.length;
      //console.log('this.totalCount clearDateFilters : ' + this.totalCount);
      if(this.previousFilterfield!=undefined && this.previousFilterfield!="")
      {
        this.sort(this.previousFilterfield,false)
      }
  ...
  clearAllFilters(){
    ...
    // this.pageNo=1;
    let pageNumber:number=1;
   // console.log("after all reset variables", this.unFilteredBlotterData)

    let backupData = this.excelDataFlag ? this.exportData : this.unFilteredBlotterData;
    //console.log("after all reset backupData", this.unFilteredBlotterData,backupData)
    this.formatData=backupData.map(item=>item["Payment_Type"]).sort((a,b) => a > b ? 1 : -1)
      this.formatData=[...new Set(this.formatData)];
      //console.log("FORMAT ",this.formatData)
    ...
    //   console.log("Edege Colour ",temp)
      ...
      this.currencyData=backupData.map(item=>item["Currency"]).sort((a,b) => a > b ? 1 : -1);
      this.currencyData=[...new Set(this.currencyData)];
      console.log("after all reset this.currencyData", this.unFilteredBlotterData,this.currencyData)
      // this.productTypeData=backupData.map(item=>item["PayOff"]).sort((a,b) => a > b ? 1 : -1);
      // this.productTypeData=[...new Set(this.productTypeData)];

      this.ulTypeData=backupData.map(item=>item["Underlying"]).sort((a,b) => a > b ? 1 : -1);
      this.ulTypeData=[...new Set(this.ulTypeData)];
      console.log("after all reset this.ulTypeData", this.unFilteredBlotterData,this.ulTypeData)
      this.groupArr = [];
      this.clientArr = [];
      this.intRefArr = [];
      this.typeArr = [];
      ...
      console.log("Group Arr" + this.groupArr,[...new Set(this.initialData.map(item=>item["Group"]))].filter(el => el !== ''));
      [...new Set(this.initialData.map(item=>item["Client"]))].filter(el => el !== '').sort().forEach(el =>{
        let newObj = {
          value : el,
          checked : true
    ...
    if(this.excelDataFlag){
      ...
      this.setSlidersValue();
      this.totalCount= backupData.length;
      //console.log('this.totalCount clearAllFilters : ' + this.totalCount);
      if(this.previousFilterfield!=undefined && this.previousFilterfield!="")
      {
        this.sort(this.previousFilterfield,false)
      }
  ...
  async TimelinePopup(item) {
    try {
      ...
      await this.portfolioService.GetObservationTableData(item.External).then((resp) => {
        this.obsTableData = <object[]>resp;
      });
      await this.portfolioService.GetHistoricalTableData(item.External).then((resp) => {
        this.histTableData = <object[]>resp;
      });
      await this.portfolioService.GetCancellationData(item.External).then((resp) => {
  ...
  async sort(filterfield,flip?:boolean) {
    ...
    switch (filterfield) {
      // To autocall case
      case 'External':
        id=document.getElementById('External')
        if (flip)
        {
      ...
      case 'Trade_Date':
        ...
        id=document.getElementById('Trade_Date')
        if (flip)
        {
      ...
      case 'Fixing_Date':
        console.log("CASE PYSMBOL", filterfield)
        id=document.getElementById('Fixing_Date')
        if (flip)
        {
      ...
      case 'Payment_Date':
        console.log("CASE PYSMBOL", filterfield)
        id=document.getElementById('Payment_Date')
        if (flip)
        {
      ...
      case 'Maturity_Date':
        console.log("CASE Maturity_Date", filterfield)
      ...
      case 'Flow':
        console.log("CASE PYSMBOL", filterfield)
        id=document.getElementById('Flow')
        if (flip)
        {
  ...
  sortTable<T>(data: T[], key, reverse: boolean = false){
    ...
    .sort((a: any, b: any) => {
      ...
      if ((parseFloat(a[key].replace(/,/g, '')) ? parseFloat(a[key].replace(/,/g, '')) : 0) < (parseFloat(b[key].replace(/,/g, '')) ? parseFloat(b[key].replace(/,/g, '')) : 0)) {
          return reverse ? -1 : 1;
      } else if ((parseFloat(a[key].replace(/,/g, '')) ? parseFloat(a[key].replace(/,/g, '')) : 0) > (parseFloat(b[key].replace(/,/g, '')) ? parseFloat(b[key].replace(/,/g, '')) : 0)) {
          return reverse ? 1 : -1;
      }
      function numberWithCommas(x) {
        return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    }
  //Added to add commas to Flow column values after sorting
    if(key == 'Flow'){
      a[key] = numberWithCommas(parseFloat(a[key].replace(/,/g, '')));
      b[key] = numberWithCommas(parseFloat(b[key].replace(/,/g, '')));
    }
    }  
    });
  ...
  async Get_Portfolio_Scenarios(oneByOne, underlying,spot)
  {
    ...
    if(mainData1)
    {
      ...
      //this.setSlidersValue();
      this.totalCount=this.blotterData.length;
      //console.log('this.totalCount Get_Portfolio_Scenarios : ' + this.totalCount);
      let mainData = blotterData;
      this.unFilteredBlotterData = mainData;
      this.filteredData = this.unFilteredBlotterData;
      //this.onKeydownRowsPerPage(10);
  ...
  nextBtnClicked(data?:any) {
    try {
      ...
      let begin:number;
      let end:number;
      //console.log('this.totalCount nextBtnClicked : ' + this.totalCount);
      //this.setMPGData(true);
      ...
      const viewportElement = this.viewport.elementRef.nativeElement;
      viewportElement.scrollTop = 0;
      console.log("SLICED ARRAY 1",this.blotterData)
      }
  ...
  onKeydownRowsPerPage(value: any, data?:any,pageNumber?:number) {
    try {
      ...
      // this.blotterData = this.unFilteredBlotterData?.slice(begin, end);
      console.log("SLICED ARRAY 2",this.blotterData);
      // Reset the viewport's scrollTop to 0 to display the top of the next page
      const viewportElement = this.viewport.elementRef.nativeElement;
      viewportElement.scrollTop = 0;
      //this.blotterData = this.sortTable(this.blotterData, "Start_Date");
  ...
  onKeydownPageNo(value: any,data?:any) {
    try {
      let begin:number;
      let end:number;
      //console.log('this.totalCount onKeydownPageNo : ' + this.totalCount);
      ...
      // this.blotterData = this.unFilteredBlotterData?.slice(begin, end);
      //this.blotterData = this.sortTable(this.blotterData, "Start_Date");
      console.log("SLICED ARRAY 3",this.blotterData);
      // Reset the viewport's scrollTop to 0 to display the top of the next page
      const viewportElement = this.viewport.elementRef.nativeElement;
      viewportElement.scrollTop = 0;
      }
  ...
  ExportTables() {
    try {
      ...
      const selectors = ['.obs-table table', '.hist-table table'];
      const sheetNames = ['Observation Table', 'Historical Performance'];
      const file = `ObservationTable_${this.selectedItem['External']}`;
      // remove 
      const removedElements: HTMLElement[] = [];
      removedElements.push(document.querySelector('.obs-table table tbody tr:last-child td:first-child div>div'));
      removedElements.push(document.querySelector('.obs-table table tbody tr:last-child td:nth-child(2) div>div'));
      ...
      document.querySelector('.obs-table table tbody tr:last-child td:first-child div').appendChild(removedElements.shift());
      document.querySelector('.obs-table table tbody tr:last-child td:nth-child(2) div').appendChild(removedElements.shift());
      if(this.histTableData.length > 2) {
  ...
  async getDataForExport()
  {
    ...
    this.FilterForAll();
    this.exportData = await this.portfolioService.GetInteractivedata(param); //commented temporiliy
     ...
     // console.log("Filtered EXPORT DATA", this.exportData);
      ...
      this.exportData.forEach((item) => {
        ...
        assetsExcelData.push({
          ...
          'Fixing Date': item.Fixing_Date,
          'Payment Date': item.Payment_Date,
          'Maturity':item.Maturity_Date,
          'Flow':item.Flow ,
          'Currency':item.Currency,
          'Contract':item.Contract,
          'TradeNumb (%)':item.TradeNumb,
        });
      // }
          
      });
      this.excelService.exportAsExcelFile(assetsExcelData, 'SP_Payments_Recon'+this.datepipe.transform(new Date(),'dd-MMM-yyyy HH:mm:ss'));
    } catch (error) {
      console.log(error);
    }
  }
  ...
  {
    this.viewList=[];
    let PortfolioViews: any= await this.portfolioService.GetPortFolioViews(AppConfig.settings.oRes?.userID,"SPR");
  ...
  async GetPortfolio_View_Details(forDelete?:boolean, delViewName?:string)
  {
    ...
    let PortfolioViews= await this.portfolioService.GetPortFolioViewDetails(AppConfig.settings.oRes?.userID,ViewName,"SPR");
  ...
  async SavePortfolio_View_Details()
  {
    ...
    // else{
      ...
      let strXml= "<Details><Record><ISIN_String/><Client>N3KY</Client><J_Internal>F000000258</J_Internal><Tutela/><Subtutela/><Group/><Type/><Branch/><Territory/><Zone/><Payment_Type/><Currency>EUR</Currency><Fixing_Date/><Payment_Date/><Record_per_page>50</Record_per_page><Sort>Notional</Sort><Sort_direction>Descending</Sort_direction></Record></Details>";
      this.generateXML();
      let PortfolioViews= await this.portfolioService.SavePortFolioViewDetails(AppConfig.settings.oRes?.userID,this.viewName,this.viewXML,"SPR","","","");
  ...
  async UpdatePortfolio_View_Details(){
    ...
    let PortfolioViews= await this.portfolioService.SavePortFolioViewDetails(AppConfig.settings.oRes?.userID,this.viewName,this.viewXML,"SPR",this.portolioDetails['PV_ID'],"","");
    ...
    let filterXML=
      {
        ...
        "Payment_Type": this.selectedFormat, //Added by Apurva K||12-Mar-2024
        "Currency": this.selectedCurrency,
        "Payoff": this.selectedPType,
        "Issue_From":this.FixingStartDate,
        "Issue_To":this.PaymentStartDate,
  ...
  setFilter()
  {
        ...
        this.selectedFormat= this.portolioDetails["Payment_Type"];
        ...
        // this.FixingDateDisplay= this.datepipe.transform(this.portolioDetails["Issue_From"], 'yyyy-MM-dd')? this.datepipe.transform(this.portolioDetails["Issue_From"], 'yyyy-MM-dd') :""; //Added by Apurva K||05-Feb-2024
        // this.FixingStartDate = this.datepipe.transform(this.FixingStartDate, 'dd-MMM-yyyy')? this.datepipe.transform(this.FixingStartDate, 'dd-MMM-yyyy'):"" ; //Added by Apurva K||05-Feb-2024
        ...
        this.FixingStartDate = this.datepipe.transform(this.portolioDetails["Issue_From"], 'yyyy-MM-dd')? this.datepipe.transform(this.portolioDetails["Issue_From"], 'yyyy-MM-dd') :"";
        this.FixingDateDisplay = this.datepipe.transform(this.FixingStartDate, 'dd-MMM-yyyy')? this.datepipe.transform(this.FixingStartDate, 'dd-MMM-yyyy'):"" ;
        this.PaymentStartDate = this.datepipe.transform(this.portolioDetails["Issue_To"], 'yyyy-MM-dd')? this.datepipe.transform(this.portolioDetails["Issue_To"], 'yyyy-MM-dd') :"";
        this.PaymentDateDisplay = this.datepipe.transform(this.PaymentStartDate, 'dd-MMM-yyyy')? this.datepipe.transform(this.PaymentStartDate, 'dd-MMM-yyyy'):"" ;
        //console.log("this.FixingStartDate:",this.FixingStartDate,"this.PaymentStartDate:",this.PaymentStartDate)
