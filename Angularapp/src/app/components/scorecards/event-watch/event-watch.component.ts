...
@Component({
  ...
  styleUrls: ['./event-watch.component.scss']
})
...
export class EventWatchComponent implements OnInit {
  ...
  async IndexMaster() {
    try {
      ...
      const apiMethod = `Scorecards/GetFastEventsMontlyGridDatasp`;
  ...
  ExportToExcel(strNexPrev: string) {
    try {
      ...
      const fileName = `EventWatch_${monthName}_${this.fastFrontendService.selectedYear}.xlsx`;
