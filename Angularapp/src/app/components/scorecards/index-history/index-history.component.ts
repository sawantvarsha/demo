...
@Component({
  selector: 'app-index-history',
  templateUrl: './index-history.component.html',
  styleUrls: ['./index-history.component.scss']
})

export class IndexHistoryComponent implements OnInit {
  ...
  GenerateTableHeaders(len: number) {
    try {
      ...
      this.columnTargetValues.push({ key: 'iM_Index_Code', displayName: 'Index Code' });
  ...
  async IndexMaster() {
    try {
      ...
      const params = {
       ...
       "strIndexCode": this.fastFrontendService.indexSearch
      }
      const apiMethod = `Scorecards/GetIndexHistoryGridDatasp`;
  ...
  ExportToExcel(strNexPrev: string) {
    try {
      ...
      const fileName = `IndexHistory_${monthName}_${this.fastFrontendService.selectedYear}.xlsx`;
