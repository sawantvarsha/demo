...
@Component({
  selector: 'app-greeks-aggregation',
  templateUrl: './greeks-aggregation.component.html',
  styleUrls: ['./greeks-aggregation.component.scss']
})

export class GreeksAggregationComponent implements OnInit {
  ...
  async IndexMaster() {
    try {
      ...
      const params = {
	   "strISIN": this.fastFrontendService.indexSearch,
	   "strFilter": "Delta",
      ...
      const apiMethod = `Scorecards/GetLifeCycleScorecardGridDatasp`;
  ...
  ExportToExcel(strNexPrev: string) {
    try {
      ...
      const fileName = `GreeksAggregation_${monthName}_${this.fastFrontendService.selectedYear}.xlsx`;
