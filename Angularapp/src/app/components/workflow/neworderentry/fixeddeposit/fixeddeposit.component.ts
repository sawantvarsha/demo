...
export class FixeddepositComponent implements OnInit, OnDestroy {
  ...
  isUserRM: boolean;
  userType: string;
  RedirectToFixedDeposit: Subscription;
  isValidated: any;
  constructor(public afs: WorkflowApiService, public cfs: CommonApiService, private cafs: CustomerApiService, private ccfs: CustomerCommonfunctionsService, public homeapi: HomeApiService) {
  ...
  ngOnInit() {
    ...
    // this.cafs.GettCustomerAccountDetails(this.entityUser).subscribe((res: any) => {
    ...
    //     this.dealSave.cif = res[0].misc10;
    ...
    this.RedirectToFixedDeposit = this.ccfs.redirecttoCashDepositObserver.subscribe((res) => {
      if (res !== '') {

        this.Customer = this.homeapi.CustomerName;
        this.dealSave.PortfolioID = res.Param3;
        // this.updatePortfolio();
        this.dealSave.accountType = 'Saving';
        this.dealSave.sourceAcNo = res.Param1;
        // this.dealSave. = res.CustomerID;
        this.dealSave.currency = res.Param4;
        this.successMsg = '';
      }
    });
  ...
  ngOnDestroy() {
    try {
      ...
      if (this.RedirectToFixedDeposit) this.RedirectToFixedDeposit.unsubscribe();
      this.clearMessages();
      this.PortfolioTypes = [];
      this.AccountType = [];
    } catch (Exception) {
  ...
  selectedInterestType() {
    ...
    // Commented by Ketan S asked by Udit T on 24-AUG-2021 Start
    ...
    // Commented by Ketan S asked by Udit T on 24-AUG-2021 END
  ...
  getMaturityInfo() {
    ...
    this.afs.getMaturityDays(tenorObj).then(response => {
      try {
        ...
        console.log('Error occured in fixeddeposit while loading dates as :', ex);
      }
      //console.log('Maturity Amount : ', response);
    }).catch(err => console.log(err));
  }
    ...
    if (res.length > 0) {
      ...
      for (const item of res) {
        if (!map.has(item.PortfolioName)) {
          ...
          this.PortfolioTypes.push({ Type: item.PortfolioName });
        }
      }
      this.dealSave.PortfolioID = this.PortfolioTypes[0].Type;
      this.updateRMPortfolioDetails();
    }
  ...
  validation() {
    ...
    if (
      ...
      this.Message = 'Account with selected currency not present.';
      return false;
    } else {
      if (
  ...
  filterCcy() {
    ...
    //   }
    // });
    this.RePayaccounts.forEach(ele => {
      if (this.dealSave.sourceAcNo === ele.accountnumber) {
        this.dealSave.currency = ele.Accountcurrency;
      }
    });
  ...
  updateRMPortfolioDetails() {
    this.RePayaccounts = [];
    this.currencyList = [];
    // this.PortfolioTypes = [];
    this.selectedCustomerDetails.forEach(element => {
      // this.PortfolioTypes.push({ Type: element.PortfolioName });
      this.RePayaccounts.push({ accountnumber: element.AccountNo, Accountcurrency: element.SICurrency });
      this.currencyList.push(element.SICurrency);
    });

    this.dealSave.sourceAcNo = this.RePayaccounts[0].accountnumber;
    this.dealSave.currency = this.currencyList[0];
  }

  FilterAccountNumbersRM() {
    this.RePayaccounts = [];
    this.selectedCustomerDetails.forEach(ele => {
      if (this.dealSave.PortfolioID === ele.PortfolioName) {
        this.dealSave.sourceAcNo = ele.accountnumber;
        this.dealSave.currency = ele.SICurrency;
      }
    });
  }
