...
Array.prototype.BENSortBy = function (p): Array<any> {
  try {
    if (this !== undefined) {
      if (this.length > 0) {
...
export class BENComponent implements OnInit, OnDestroy {
  @ViewChild('focusable') namefield: ElementRef;
  orderdeatilsflag: boolean;
  ngOnDestroy(): void {
  ...
  ngOnInit() {
    ...
    setTimeout(async () => {
      try {
        ...
        this.remainingNotional = '1,000,000.00';
        this.totalNotional = '1,000,000.00';
        const ccyRes = await this.apifunctions.loadCurrency();
        ...
        this.commonfunctions.BENSignalRPrices.subscribe((prices) => {
          this.AllPrices = [];
          try {
            if (prices) {
              this.sortedAllPrices = [];
              for (let i = 0; i < prices.length; i++) {
                if (prices[i].BENOUT > 0) {
                  for (let k = 0; k < this.lpArr.length; k++) {
                    if (prices[i].EP_ER_QuoteRequestId === this.lpArr[k].rfq) {
                      this.lpArr[k].price = prices[i].BENOUT;
              ...
              this.sortedAllPrices = this.AllPrices.BENSortBy('price');
  ...
  BENPrice(PP, pricingID) {
    try {
      console.log(PP, pricingID);
      this.clearFlag = false;
      // this.Token = this.apifunction.AuthenticateUser();
      const webMethod = this.interfaceUrl + 'EQC/BEN/Quote';
      const that = this;
      ...
      const parameters = {
        ...
        Tenor: this.Tenor,
        PricePercentage: this.IBPrice,
        CashCurrency: this.ddlNoteCcy,
        Exchange1: this.Exchange(),
        SettlementDays: this.DaysCount,
          ...
          if (!that.clearFlag) {
            ...
            if (that.PPDetails !== '') {
              that.interval = setInterval(() => {
                if (that.timeLeft > 0) {
                  that.BENPriceResponse(that.PPDetails);
  ...
  BENPriceResponse(PPDetails) {
    try {
      ...
      const webMethod = this.interfaceUrl + 'EQC/BEN/QuoteResponse';
      const that = this;
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (that.timeLeft > 0) {
            that.commonfunction.setBENReceivedPrices(that.Prices);
          }
  ...
  bookOrder(
    ...
    try {
      ...
      this.apifunction.RMList('Mobile1').subscribe((data) => {
        ...
        this.apifunction
          .EQCRef('BEN', this.Tenor, this.TenorType, 'Mobile1')
          .subscribe((data) => {
            data = JSON.parse(data['responseData']);
            this.eqcRef = data;
  ...
  reset() {
    try {
      this.timeLeft = 0;
      clearInterval(this.interval);
      this.commonfunction.setBENReceivedPrices({});
