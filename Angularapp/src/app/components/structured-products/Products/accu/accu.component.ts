...
Array.prototype.AQsortBy = function (p): Array<any> {
  try {
    ...
    if (this !== undefined) {
      if (this.length > 0) {
        if (this[0].solvefor.toUpperCase().trim() !== 'UPFRONT') {
          return this.slice(0).sort(function (a, b) {
...
@Component({
  selector: 'app-accu',
  templateUrl: './accu.component.html',
  styleUrls: ['./accu.component.scss'],
})
export class AccuComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      try {
        this.reset();
        this.commonfunction.ClearPricesFromMultiToDealEntry(); // clear existing price observer
        this.Tenor = 6;
        this.upfront = '0.50';
        this.IBPrice = '99.50';
        this.KO = '102.00';
        ...
        this.commonfunction.Currency.subscribe((cu) => (this.ddlNoteCcy = cu)); // subscribe to currency
        ...
        this.commonfunctions.AQSignalRPrices.subscribe((prices) => {
          // subscribe to Price
          this.AllPrices = [];
          try {
            if (prices) {
  ...
  // change settlement days as per setttlement weeks
  ...
  // share dropdown set index for scrolling
  ChangeIndex(e) {
    try {
      console.log(e);
      //console.log('key');
  ...
  // funtion to generate quote for pricing
  AccumPrice(PPID, pricingID) {
    try {
      console.log(PPID, pricingID);
  ...
  // funtion to generate quote response(pricing)
    ...
    try {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (that.timeLeft > 0) {
            that.commonfunction.setAQReceivedPrices(that.Prices);
          }
  ...
  bookOrder(
    ...
    try {
      ...
      this.apifunction.RMList('Mobile1').subscribe((data) => {
        ...
        this.rmList = data;
        this.apifunction
          .EQCRef('ACUM', this.Tenor, this.TenorType, 'Mobile1')
          .subscribe((data) => {
            data = JSON.parse(data['responseData']);
            this.eqcRef = data;
  ...
  // reset value after paramenter change
  reset() {
    try {
      this.timeLeft = 0;
      clearInterval(this.interval);
      this.commonfunction.setAQReceivedPrices({});
  ...
  scrollWin() {
    try {
      ...
      this.lblOrderBlotter = '- Order Blotter';
      this.fnEQConnectOrderHistory('ACCDAC', 'ACCUMULATOR', 'ACCDACBlotter');
      setTimeout(function () {
  ...
  fnGetLPList() {
    try {
      this.apifunction.GetLPList('AQ').subscribe((Res) => {
        if (Res) {
          this.apifunction.SetToken(Res['token']);
          this.lpArr = [];
          let LPArray = JSON.parse(Res['responseData']);
          LPArray = LPArray.AQSortBy('Link_Provider_Name');
