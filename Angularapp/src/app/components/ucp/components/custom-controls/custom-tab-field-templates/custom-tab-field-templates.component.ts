...
export class CustomTabFieldTemplatesComponent implements OnInit  {
  ...
  ngOnInit() {
    ...
    try{
      ...
      if ((this.control.dataType === "AMOUNT" ||this.control.dataType === "NUMBER")) { //added by devesh as amount was not formatting on value change,same code was present in field template but missing here // Changed by OnkarE on 09-Oct-2023
  ...
  formatKLMB(value: string, precision = 2) {
    try{
    // Added if condition by OnkarE on 10-Oct-2023
