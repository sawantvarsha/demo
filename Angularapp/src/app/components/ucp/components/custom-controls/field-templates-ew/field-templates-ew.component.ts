...
@Component({
  selector: 'app-field-templates-ew',
  templateUrl: './field-templates-ew.component.html',
  styleUrls: ['./field-templates-ew.component.scss'],
  styles: [
    `
      mat-form-field {
...
export class FieldTemplatesEWComponent implements OnInit {
  @Input() control: any;
  @Input() dropdowns: any;
 ...
 // mandatoryImageUrl = require('../../../../../assets/mandatory.png');
  ...
  udfUnderline = false;
  dynamicCSSList: any[] = [];
  lblWidthApplied: string;
  colWidthApplied: string;
  totalWidthApplied: string;
  ...
  ngOnInit() {
    ...
    this.lblWidthApplied =  this.lblWidthToApply + "px"
    this.colWidthApplied =  this.colWidthToApply + "px"

    var addWidth = Number(Number(this.layoutLblWidth) + Number(this.layoutColWidth))
    this.totalWidthApplied =  addWidth + "px";
  ...
  checkUDFVisibility(index: number) {
    try{
    const mod = index % 50;
    console.log("field templates: ", index, mod, this.ColSize, this.RowSize)
  ...
  formatKLMB(value: string, precision = 2) {
    ...
    if(precision == 2)
      ...
      value = parseFloat(value).toLocaleString('en-US', { minimumFractionDigits: precision, maximumFractionDigits: precision }); //Modified by RajeshC || 13-10-2023 || Notional Formatting
    else 
