...
export class CustomerGridFieldTemplatesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log("customer Grid Component is loading... ", this.control.displayName, this.control, this.index, this.dropdowns);
      ...
      if(Number(this.control.ucgM_Column_Width) == 0 || this.control.ucgM_Column_Width == ""){
        this.appliedWidth = '120px';
      }
      else {
  ...
  formatKLMB(value: string, precision = 2) {
    try{
    //Added by RajeshC after discussing with OnkarE || CustomerGrid notional formatting
      if(precision == 0){
        precision = 2;
      }
      ...
      if(precision == 2)
        ...
        value = parseFloat(value).toLocaleString('en-US', { minimumFractionDigits: precision, maximumFractionDigits: precision });
      else 
