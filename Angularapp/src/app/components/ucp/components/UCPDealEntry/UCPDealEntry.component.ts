	...
	import { UCPMODE } from 'src/app/components/ucp/services/UCPMODE.enum';
	...
	import { AutocompleteService } from 'src/app/components/ucp/services/autocomplete.service';

	import { LoaderService } from 'src/app/components/ucp/services/LoaderService';
	...
	DefaultValue:any;
		...
		searchArrays: any[] = [];
		shareArray: any[] = [];
		ddlTypes: any[] = []; // Changed by OnkarE on 10-Oct-2023
		functions: any[] = [];
		...
		iNoteDealID: string;
		moduleControl = new UntypedFormControl();
		moduleData: any;
		schemeControl = new UntypedFormControl();
		schemeData: any;
		templateControl = new UntypedFormControl();
		templateData: any;
		bookingModelControl = new UntypedFormControl();
		bookingModelData: any[];
		isHedgingHidden = false;
		isSubTemp = false;
		subTempControl = new UntypedFormControl();
		subTemplate: any;
		subTemplateData: any;
		...
		private loaderService: LoaderService,public matDialog: MatDialog, private renderer:Renderer2, public themeService: ThemeserviceService) {
			...
			//console.log(baseUrl);
			//Modified by RajeshC || Santander || GetEntity || 13-10-2023
			this.Login_Id = sessionStorage.getItem('FinIQUserID') ? sessionStorage.getItem('FinIQUserID') : 'SantanderDealer1';
			this.userGroup = sessionStorage.getItem('FinIQGroupID') ? sessionStorage.getItem('FinIQGroupID') : 'SantanderDealer'; /// Added by OnkarE to get actual user's login id after merging deal entry in setup - on 25-Jan-2021
		...
		async ngOnInit() {
			...
			// })
			const that = this;
			this.loader = true; // Added by OnkarE on 24-Aug-2023 JIRA:UWI-387
			this.getentityData();
			...
			/// Modified this block by OnkarE on 09-Oct-2023
			// this.api.udfDropdownDataObserver.subscribe(
			// 	(res: any[]) => {
			// 		const start = performance.now();
			// 		console.log("set udf dd: before: ", res);
			// 		if (res) {
			...
			await this.api.GetUDFFieldDropdownValues(this.selectedEntity, 0, this.Login_Id).then( //to load all functions data to be executed on template change as list remains same irrespective of subtemplate //removed async await here             
					(response: any) => {
						if (!this.isNullOrUndefined(response.response)) {
							var finalResp = response.response
							finalResp = finalResp != null ? finalResp.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : finalResp
							finalResp = finalResp != null ? finalResp.map(({
								iD:id,
								...rest
							}) => ({
								id,
							...
							if (finalResp) {
									that.ddlTypes = [...new Set(finalResp.map(f => f.type))];
									that.ddlTypes.forEach((d: any) => {
										that.defaultDropdowns[d] = finalResp.filter((c: any) => c.type === d).map((c: any) => ({
											code: c.code.trim()
										}));
										that.defaultSearchArrays[d] = finalResp.filter((c: any) => c.type === d);										
									});
									//that.defaultSearchArrays = finalResp;
									console.log('set udf dropdowns', that.defaultSearchArrays);
		...
		async getAllTemplatesforsearch(){
			try{
			//this.templates == []
			// Commented by OnkarE on 09-Oct-2023
				...
				if (UCPMode === 'UCPQEN') {
					...
					try {
						this.api.GetStaticAccessMatrix(this.userGroup, this.Login_Id, String(this.selectedEntity)).then(
							(response: any) => {
								// console.log(response);
								if (!this.isNullOrUndefined(response) && !this.isNullOrUndefined(response.GetStaticAccessMatrixDataResult)) {
									// const res = response.response;
									var res = response.GetStaticAccessMatrixDataResult ? response.GetStaticAccessMatrixDataResult : response;
									res = res != null ? res.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj}) : res
									res[0] = JSON.parse(JSON.stringify(res[0]).replace(/uSAM/g, 'usaM'))
									this.SMADataObj = res != null ? res[0] : null;
									// console.log(res);
									if (this.SMADataObj != null) {
										that.EntityText = res[0].usaM_LabelEntityText;
		...
		getDealDetails() {
			...
			try {
				...
				this.api.GetDealDetails(this.iNoteMasterID ? this.iNoteMasterID : "", this.iNoteDealID ? this.iNoteDealID : "", Number(this.entityId)).subscribe(
					(response: any) => {
						if (!this.isNullOrUndefined(response.GetDealDetailsResult)) {
							var res = response.GetDealDetailsResult;
							res = res != null ? res.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : res
	...
	// Changes by OnkarE on 10-Oct-2023
		async getentityData() {
			// try {
			...
			// 			if(!this.isNullOrUndefined(response.GetEntityResult)) {
							
			// 				var res = response.GetEntityResult ? response.GetEntityResult : response;
			// 				res = res != null ? res.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : res
			// 				this.Entityddl = res;
			// 				this.entityData = res[0].entity_Name;
			// 				this.selectedEntity = res[0].entity_ID;
			...
			try{
				//this.Login_Id = "SantanderDealer1";
				//console.log("Login_ID", this.Login_Id);
	
				await this.api.getEntityData(this.Login_Id).then(
					(response: any) => {
						if (response.status === Status.Success) {
							...
							this.entityData = res[0].entity_Name;
							this.selectedEntity = res[0].entity_ID;
							console.log("Selected Entity is:: ", this.selectedEntity,this.Entityddl,this.entityData)
						}
					}
				);
			}
			...
			try {
				...
				await this.api.getModuleData(this.userGroup, this.sUCPMode, this.Login_Id, String(this.selectedEntity)).then(
					(response: any) => {
						...
						// Changed by RajeshC on 09-Oct-2023
						if(!this.isNullOrUndefined(response.response)) {
		...
		async GetCustomerNames() {
			...
			// Commented by OnkarE
				...
				// 	}
				// );
				console.log("INGETCUSTOMER_NAMES");
			} catch (error) {
				console.error(error);
		...
		getSchemeData(CBOModule: any) {
			try {
				...
				this.api.getSchemeData(CBOModule, this.sUCPMode, this.Login_Id, String(this.selectedEntity), this.userGroup).then(
					(response: any) => {
						// Changed by OnkarE on 09-Oct-2023
						if (!this.isNullOrUndefined(response.response)) {
							//const res = response.response;
		...
		getTemplateData(schemeId: any) {
			try {
				...
				this.api.getTemplateData(schemeId.toString(), this.Application_type, this.sUCPMode, this.Login_Id, String(this.selectedEntity), this.userGroup).then(
					(response: any) => {
					// Changed by RajeshC on 09-Oct-2023
						console.log("GETTEMPLATEDATA",response.response);
						if (!this.isNullOrUndefined(response.response)) {
							//const res = response.response;
							// console.log(res);
							var res = response.response ? response.response : response;
			...
			try {
				...
				this.api.getTemplateMapping(this.template.template_Id,Apptype, this.template.template_Name, this.sUCPMode, this.Login_Id, this.userGroup).then(
					...
					// Changed by RajeshC on 09-Oct-2023
						if (!this.isNullOrUndefined(response.response)) {
							var res = response.response;
							console.log("getTemplateMapping",res);
							res = res.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; })
				...
				await this.api.GetFunctionDetails(this.template.template_Id).then( //to load all functions data to be executed on template change as list remains same irrespective of subtemplate //removed async await here             
					(response: any) => {
						...
						if (!this.isNullOrUndefined(response.response)) {
							...
							console.log("Function Details Response is .........", response);
							var finalResp = response.response
							finalResp = finalResp != null ? finalResp.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : finalResp
							finalResp = finalResp != null ? finalResp.map(({
								uFD_Template_ID: ufD_Template_ID,
								uFD_Function_TYpe: ufD_Function_TYpe,
								uFD_Function_Name: ufD_Function_Name,
								uFD_Function_Output_Form: ufD_Function_Output_Form,
								uFD_Target_Field: ufD_Target_Field,
								uFD_DB_Name: ufD_DB_Name,
								uFD_Class_Name: ufD_Class_Name,
								uFD_CSV_Input: ufD_CSV_Input,
								uFD_Tab_Name: ufD_Tab_Name,
								uFD_Tab_Type: ufD_Tab_Type,
								uFD_Cascade_YN: ufD_Cascade_YN,
								uFD_Remote_Acnt_LogIN: ufD_Remote_Acnt_LogIN,
								uFD_ID: ufD_ID,
								uFD_cacheYN: ufD_cacheYN,
								uFD_Reference_Pointer: ufD_Reference_Pointer,
								uFD_CSV_Output: ufD_CSV_Output,
								uFD_Reference_Object_CSV: ufD_Reference_Object_CSV,
								uFD_SampleRequest: ufD_SampleRequest,
								uFD_SampleResponse: ufD_SampleResponse,
								uFD_Parameter_CSV: ufD_Parameter_CSV,
								uFD_Mandatory_CSV: ufD_Mandatory_CSV,
								...rest
							}) => ({
								ufD_Template_ID,
								ufD_Function_TYpe,
								ufD_Function_Name,
								ufD_Function_Output_Form,
								ufD_Target_Field,
								ufD_DB_Name,
								ufD_Class_Name,
								ufD_CSV_Input,
								...
								ufD_Tab_Type,
								ufD_Cascade_YN,
								ufD_Remote_Acnt_LogIN,
								ufD_ID,
								ufD_cacheYN,
								ufD_Reference_Pointer,
								ufD_CSV_Output,
								ufD_Reference_Object_CSV,
								ufD_SampleRequest,
								ufD_SampleResponse,
								ufD_Parameter_CSV,
								ufD_Mandatory_CSV,
		...
		async FillSubTemplate() { //put async await here as response was not being received before read layouts function in changesubtemplate
			try {
				...
				await this.api.GetSubTemplate(String(this.template.template_Id)).then(response => {
					if (!this.isNullOrUndefined(response.GetSubTemplateResult)) {
						var res = response.GetSubTemplateResult;
						res = res != null ? res.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : res
						res = res.map(({
							tOM_Sub_Template_Name: toM_Sub_Template_Name,
							tOM_ID: toM_ID,
							tOM_Template_ID: toM_Template_ID,
							...rest
						  }) => ({
							toM_Sub_Template_Name,
							toM_ID,
							toM_Template_ID,
							...rest
						  }));
						...
						console.log("Sub template data: ", this.subTemplateData, this.subTemplate);
		...
		SetStaticValuesFromTemplateCodeInMigrationMode() {
			try {
				...
				this.api.GetStaticData(Number(this.iNoteMasterID)).then(
					(response: any) => {
						if (!this.isNullOrUndefined(response.GetStaticDataResult)) {
							response.GetStaticDataResult = response.GetStaticDataResult != null ? response.GetStaticDataResult.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : response.GetStaticDataResult
							var  oStaticDataDetails = response.GetStaticDataResult[0];
							this.iProductId = Number(oStaticDataDetails.product_Id)
							this.iSchemeCode = Number(oStaticDataDetails.scheme_code)
							this.iTemplateID = Number(oStaticDataDetails.template_ID)
							this.iDefaultProductId = Number(oStaticDataDetails.product_Id)
			...
			try {
				...
				this.api.GetIntersectionMappingDetails(templateID).subscribe(
					(response: any) => {
						...
						if (!this.isNullOrUndefined(response.GetIntersectionMappingDetailsResult)) {

							console.log("Intersection data");
							console.log(response);
							this.intersections = response.GetIntersectionMappingDetailsResult;
	...
	async decCustGridTableRowCount(row:any) {
		...
		try
		{
				...
				if (this.Embedded_Control.toUpperCase() == "MULTIUCP" && !this.isNullOrUndefined(sessionStorage.getItem('oFieldDetails'))) {
					...
					await this.api.ReadFieldDetailsAngular(this.template.template_Id, this.template.template_Code, this.Application_type, this.sUCPMode, this.iTemplateSrNo, this.subTemplate.toM_ID, this.Login_Id, this.userGroup,this.EditModeMappedTOWFBMSTField,this.selectedBookingModel,this.iNoteMasterID, this.isNullOrUndefined(this.QueueNo) ? 0 : Number(this.QueueNo), Number(this.entityId), this.sClickedButton ).then(
						async (response: any) => {
							const start = performance.now();
							if (!this.isNullOrUndefined(response.ReadFieldDetails_Wrapper_AngularResult)) {
								var res = response.ReadFieldDetails_Wrapper_AngularResult;

								res.udfFields = res.UDFFields ? res.UDFFields.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : res.UDFFields
								res.customTab = res.CustomTab ? res.CustomTab.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : res.CustomTab
								res.dropdowns = res.Dropdowns ? res.Dropdowns.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : res.Dropdowns
								res.oBaksetArray = res.oBaksetArray ? res.oBaksetArray.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; }) : res.oBaksetArray
								res.custGrid = [];
								res.CustGrid ? res.CustGrid.map(element => {
									element = element.map(e=>{const values = Object.values(e); const keys = Object.keys(e).map(k=>k.charAt(0).toLowerCase() + k.substr(1)); const obj = {}; keys.forEach((k,i)=> obj[k] = values[i]); return obj; })
									element = element.map(({
									uCP_Clone_Field: ucP_Clone_Field,
									uCGM_Column_Width: ucgM_Column_Width,
									uCGM_Feature1: ucgM_Feature1,
									uCGM_Feature2: ucgM_Feature2,
									dDVisibility: ddVisibility,
									dDMandatory: ddMandatory,
									dDReadOnly: ddReadOnly,
									uCGM_format: ucgM_format,
									uCGM_ColumnType: ucgM_ColumnType,
									...rest
									}) => ({
									ucP_Clone_Field,
									ucgM_Column_Width,
									...
									res.custGrid.push(element);
								}) : res.CustGrid
								res.udfFields = res.udfFields.map(({
									uDF_Feature1: udF_Feature1,
									uDF_Feature2: udF_Feature2,
									uDF_Format: udF_Format,
									uDF_FontName: udF_FontName,
									uDF_FontStyle: udF_FontStyle,
									uDF_FontSize: udF_FontSize,
									...rest
								}) => ({
									udF_Feature1,
									udF_Feature2,
								...
								res.customTab = res.customTab.map(({
									uDF_Feature1: udF_Feature1,
									uDF_Feature2: udF_Feature2,
									uDF_Format: udF_Format,
									uDF_FontName: udF_FontName,
							...
							if (this.Embedded_Control.toUpperCase()!=="MULTIUCP" && this.Embedded_Control.toUpperCase()!=="NATIVEFX") { // added by devesh on 29-jul-2020, dont load customer grid and custom tab if multiucp to make it faster else normal flow 
									...
									// Changed position of this code block - to get all common dropdwons data first and cust grid assignment later - by OnkarE on 10-Oct-2023
														...
														row.forEach((c: any) => {
															...
															if(c.type.toUpperCase() == 'COMMON DATA')
															{
																...
																if (c.sourcingLink.length) {
																	that.dropdowns[c.sourcingLink] = this.common_data.filter((common: any) => common.Type === c.sourcingLink);
																	that.searchArrays[c.sourcingLink] = this.common_data.filter((common: any) => common.Type === c.sourcingLink);
																	// Changes in below code block - added if else conditions by OnkarE on 10-Oct-2023 
																	if(that.dropdowns[c.sourcingLink].length > 0){
																	...
																	else if(that.dropdowns[c.columnName].length > 0){
																		that.dropdowns[c.columnName + "_cg_" + String(index)] = that.dropdowns[c.columnName]; 
																		that.searchArrays[c.columnName + "_cg_" + String(index)] = that.searchArrays[c.columnName];
																	}
																	//this.dropdowns[c.columnName + "_cg_" + String(index)].map(c=>c.code=c.Code);
																	//this.searchArrays[c.columnName + "_cg_" + String(index)].map(c=>c.code=c.Code);
		...
		async GetCommonDataDropdownValues() {
			...
			await tempPromise1.then(  
				(data : any) => {  
					...
					// Changed by RajeshC on 09-Oct-2023
					//this.common_data = data.GetCommonDataDropdownValuesResult
		...
		GetCustomStaticTabDetails(template_Id: number) { //added by devesh for getting static details of custom tab
			try {
				this.api.GetCustomTabDetails(template_Id).subscribe((response: any) => {
					if (!this.isNullOrUndefined(response.GetCustomTabDetailsResult)) {
						console.log(response);
						this.CustomStaticTabDetails = response.GetCustomTabDetailsResult;
						this.CustomStaticTabDetails = this.CustomStaticTabDetails.map(({
							CT_MaxRowCount: cT_MaxRowCount,
							CT_TabId: cT_TabId,
							CT_Tab_Field_Name: cT_Tab_Field_Name,
							CT_UCP_CloneInward: cT_UCP_CloneInward,
							CT_UCP_CloneOutward: cT_UCP_CloneOutward,
							Tab_Type: tab_Type,
							UCPFieldValue: ucpFieldValue,
							...rest
						  }) => ({
							cT_MaxRowCount,
							cT_TabId,
		...
		GetMappedTabsDetails() {
			...
			try {
				this.api.GetMappedTabsDetails(this.iTemplateID.toString(), "0", this.sUCPMode, this.userGroup, this.Login_Id)
				.then(
					(response: any) => {
						if (response.status === Status.Success) {
							const res = response.response;
