...
export class SpPortfolioSnapshotService {
  ...
  async GetObservationTableDTData(noteMasterId: string) {
    try {
      const url = AppConfig.settings.apiBaseUrl + `pdt/Get_ObservationData_DT`;
      return await this.http.post(url, { "note_Master_ID": noteMasterId}).toPromise().then(res => {
        return res;
      });
    } catch (error) {
  ...
  async GetCancellationData(noteMasterId: string) {
    try {
      const url = AppConfig.settings.apiBaseUrl + `pdt/Get_CancellationData`;
      return await this.http.post(url, { "note_Master_ID": noteMasterId}).toPromise().then(res => {
        return res;
      });
    } catch (error) {
